...
def sivel(data):
      ...
      if sn in dic_meta.keys() and sn in dic_phreno.keys():
         ...
         for sp in dic_phreno[sn]["spectra"]:
            if sp in meta['spectra'].keys() and  numpy.abs(meta['spectra'][sp]['salt2.phase']) < 2.5 and numpy.isfinite(dic_phreno[sn]["spectra"][sp]["phrenology.vSiII_6355_lbd"]):
               ...
               dum2 += dic_phreno[sn]["spectra"][sp]["phrenology.EWFe4800"]/dic_phreno[sn]['spectra'][sp]["phrenology.EWFe4800.err"]**2
               dum2_err += 1/dic_phreno[sn]['spectra'][sp]["phrenology.EWFe4800.err"]**2
               counter +=1
         if counter !=0:
            ...
            EWFe4800.append(dum2/dum2_err)
            EWFe4800_err.append(1/numpy.sqrt(dum2_err))

         else:
            sivel.append(float('nan'))
            sivel_err.append(float('nan'))
            EWCAIIHK.append(float('nan'))
            EWFe4800.append(float('nan'))
   ...
   EWFe4800 = numpy.array(EWFe4800)
   EWFe4800_err = numpy.array(EWFe4800_err)
