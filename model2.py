...
init = [{'EW' : EW_renorm, 'c': numpy.random.uniform(-0.25,-0.08,size=5), 'alpha': numpy.random.uniform(-0.01, 0.01, size= 5),\
    'beta':numpy.array([0.04,0.035,0.03,0.025, 0.025])+ numpy.random.uniform(-0.002,0.002,size=5),\
    'gamma':numpy.array([1.4,1.21,0.824,0.67])+ numpy.random.uniform(-0.01,0.01,size=4), \
    'k':numpy.random.uniform(-0.05, 0.05, size=nsne-1), 'mag_int': mag_renorm+numpy.random.uniform(-0.02,0.02,size=(nsne,5)), \
...
fit = sm.sampling(data=data, iter=10000, chains=4,init=init)
