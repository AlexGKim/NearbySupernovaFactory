...
parameters {
  ...
  simplex[D] R_unit;
  real<lower=0, upper=0.4> rho00;
  vector<lower=-0.4, upper=0.4>[4] rho0_;
  vector<lower=-0.10, upper=0.02>[5] rho1;
}
...
model {
  ...
  for (d in 1:D) {
      means[d] <- Delta[d] + c+ alpha*EW[d,1] + beta*EW[d,2] + rho0*R[d] + rho1*(pow(R[d],2)-1);
  }
