...
parameters {
  ...
  simplex[D] R_unit;
  real<lower=0, upper=0.2> rho00;
  vector<lower=-0.2, upper=0.2>[4] rho0_;
  vector<lower=-0.13, upper=0.13>[5] rho1;
}
...
model {
  ...
  for (d in 1:D) {
      means[d] <- Delta[d] + alpha*EW[d,1] -rho1  + beta*EW[d,2] + rho0*R[d] + rho1*pow(R[d],2);
  }
