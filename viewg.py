...
#     pp = PdfPages('outputg/magresidual_{}.pdf'.format(fi))
...
pp = PdfPages('outputg/sigma_corner.pdf')
plt.savefig(pp,format='pdf')
pp.close()
plt.close()
...
plt.title(r'log p')
pp = PdfPages('outputg/likelihood.pdf')
plt.savefig(pp,format='pdf')
pp.close()
plt.close()


with PdfPages('outputg/multipage_pdf.pdf') as pdf:
