    ...
    # for i in xrange(1):
        ...
        for j in xrange(fit['Delta'].shape[1]):
            ...
            newDelta.append(fit['Delta'][i,j] - \
                numpy.dot(c,numpy.array([fit['EW'][i,j,0],fit['EW'][i,j,1],fit['sivel'][i,j], fit['x1'][i,j], fit['k'][i,j], fit['R'][i,j], fit['ev_sig'][i]* fit['mag_int_raw'][i,j]])))
...
def fix3():
    ...
    for i in xrange(fit['Delta'].shape[0]):
        ...
        hs = numpy.array([fit['EW'][i,:,0],fit['EW'][i,:,1],fit['sivel'][i,:],fit['Delta'][i,:]])
        bigcov = numpy.cov(hs)
        hscov = bigcov[:3,:3]
        hdcov = bigcov[:3,3]
        c = numpy.dot(numpy.linalg.inv(hscov), hdcov)
        ...
        # # now do latent parameters
        latents =[fit['k'][i,:],fit['R'][i,:],fit['mag_int_raw'][i,:]]
        cofactors =[fit['gamma'][i,:],fit['rho1'][i,:],fit['ev_sig'][i] * fit['ev'][i,:]]
        newpars = []
        for lat, cof in zip(latents,cofactors):
            hs = numpy.array([fit['EW'][i,:,0],fit['EW'][i,:,1],fit['sivel'][i,:],lat])
            bigcov = numpy.cov(hs)
            hscov = bigcov[:3,:3]
            hdcov = bigcov[:3,3]
            c = numpy.dot(numpy.linalg.inv(hscov), hdcov)
        ...
        outfit['k'][i,:] = newpars[0]
        outfit['R'][i,:] = newpars[1]
        outfit['mag_int_raw'][i,:]  = newpars[2]
...
def fix1():
    ...
    for i in xrange(fit['Delta'].shape[0]):
        ...
        # # now do latent parameters
        latents =[fit['k'][i,:],fit['R'][i,:]]
        cofactors =[fit['gamma'][i,:],fit['rho1'][i,:]]
        newpars = []
        for lat, cof in zip(latents,cofactors):
            hs = numpy.array([fit['EW'][i,:,0],fit['EW'][i,:,1],fit['sivel'][i,:],lat])
            bigcov = numpy.cov(hs)
