...
Each supernova is described by its parameters $EW_{Ca}$, $EW_{Si}$, $\lambda_{Si}$, $E_{\gamma^0}(B-V)=(\gamma^0_B-\gamma^0_V)k$, and
...
\subsection{Two Extrinsic Parameters}
\color{red}
...
influence of $k$ and $k_1$  on supernova magnitudes
with probability $(1-5\times 10^{-5})$.

\color{red}
Combining the effects of both parameters, we can define a total $X$-band extinction $A_X=\gamma^0_X k + \gamma^1_X k_1$
...
estimate $R_V=A_V/(E(B-V)-zeropoint$ per supernova.  These values are shown in Figure~\ref{avebv:fig}.
