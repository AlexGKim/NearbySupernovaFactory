...
dependent on the spectral equivalent widths and two independent color parameters, to better than 
%-----
$(1-5\times10^{-5})$
%------
confidence.
The two parameters exhibit the behavior expected from the two-parameter
\citet{1999PASP..111...63F} dust model with a per-supernova scatter in
total-to-selective extinction, an effective
%-----
$R^F_V=2.43^{+0.16}_{-0.17}$
%------
for the full sample, and a difference in the average $R^F_V$ between extremely blue and red supernovae .  The derived dust law is able to reproduce the optical colors of the highly reddened SN~2014J better than the Fitzpatrick dust model.
Extending the model to allow three color parameters gives almost identical results for
the two parameters, plus a detection of a third parameter at  $>98$\% confidence.
\color{black}
\end{abstract}
...
They remain two-parameter models, with one parameter characterizing light-curve shape and the other
 ...
 whereas MLCS2k2 varies shape with additive corrections to magnitudes.}
...
\textcolor{red}{partially to intrinsic variations connected to light-curve shape, and partially}
...
\textcolor{red}{
\citet{2011MNRAS.413.3075M} argue that viewing angle is an important determinant in controlling supernova color, which is
traced by wavelength shifts in nebular emission lines; such correlations are also seen by \citet{2011A&A...534L..15C}.
}
...
\textcolor{red}{
Such studies measuring $R_V$ are being advanced with the development of flexible empirical light curve models that accommodate flexibility in multi-band colors
...
such as multiple parameters that simultaneously influence color, can be determined.
\citet{2016arXiv160904470M} take the approach of modeling the distribution of their parameters, to find that the distribution of
observed supernova magnitudes is better explained by a combination of intrinsic color dispersion and
$R_B=3.7$ dust, over dust with no color dispersion.
They draw these conclusions by using only the SALT2 fit $X_1$ parameter as the summary statistic that describes color.
}
...
extrinsic processes, which we attribute to dust.  The data used in this analysis are described in \S\ref{data:sec}.  \textcolor{red}{A
first analysis omitting the intrinsic parameter is presented in
\S\ref{model:sec}, where we show that the properties of the two extrinsic parameters
are consistent with the expectations of Milky Way-parameterized dust, and do an excellent job of fitting the data of the out-of-sample
supernova SN2014J that has been previously identified as having extreme total-to-selective extinction.
In \S\ref{model2:sec} we add to analysis the third intrinsic parameter, and find strong evidence for its influence on observed magnitudes.
...
The flux calibration is presented in \citet{2013A&A...549A...8B}.  A detailed
...
For each supernova, the magnitudes at peak brightness are regressed from data within 5-days of peak brightness
...
\textcolor{red}{SiII$\lambda 6355$ wavelength are taken from spectra  within $\pm 2.5$ days from maximum.}
...
\tabletypesize{\tiny}
\tablecaption{Supernova Spectral-Feature and Peak-Magnitude Data \textcolor{red}{TABLE MAY GO IF THERE IS A NICO PAPER}
\label{data:tab}}
\tablehead{
...
the spectral measurements, $EW_{Ca}$, $EW_{Si}$ and $\lambda_{Si}$,  $k_0$ and $k_1$ are latent parameters with no
directly associated observables.}
...
in a linear model this value is arbitrary.    The latter two exclude degenerate posterior space
associated
...
deviations $\sigma_i = \sqrt{C_{c,ii}}$ with a  Cauchy distribution prior with location
 ...
 \textcolor{red}{This prior for a covariance matrix is recommended by STAN.}
...
$N_{eff}$ is an estimate of the number of independent draws.  \textcolor{red}{We run four chains, each with 10000 iterations of which
half are used for warmup.}
The output for our data and model gives $\hat{R} \sim 1.0$ for all parameters, meaning there is no evidence for non-convergence.  The
...
\hline
\end{tabular}
\caption{68\% credible intervals for the Global Fit Parameters of the 2-Parameter Extrinsic Model \label{global:tab}}
\end{table}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Ideogram for the grey offset $\Delta$, normalized to have area equal 1.
   \label{hist:fig}}
\end{figure}
...
\end{pmatrix}.
  \label{color_cor:eqn}
 \end{equation}
Where quoted, corresponding color variances are smaller than those derived by \citet{2003A&A...404..901N, 2007ApJ...659..122J}
...
The range of SiII$\lambda$4130 equivalent widths is $\pm 20$~\AA\ whereas the width of the $B$-band is 851~\AA, so that the affect on magnitude
is
...
701~\AA, so that the affect on magnitude
is
$2.5 \log{50/701} \sim 0.08$ mags.   The implied span in $U$ magnitude is  0.21~mag, so $\alpha_0$ cannot be completely due to the line iteslf.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.5in]{output11/avebv.pdf}
   \includegraphics[width=2.5in]{output11/rv.pdf}
...
\end{equation}
The \citet{1999PASP..111...63F} model is also approximately described by a linear model in the ranges of $R_V$ and wavelength under consideration:
...
For a linear 2-parameter model, possible magnitudes are confined to a plane in the 5-dimensional UBVRI magnitude space.
...
are projected onto the Fitzpatrick values of $E^F(B-V)$ and $A^F$, up to an additive constant:  these are shown in Figure~\ref{avebv:fig}.
...
$\{k_0, k_1\}$, describe, for the most part that behaves like the $\{ E(B-V), R_V\}$
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4.5in]{output11/sn2014j.pdf} 
...
\section{Supernova Model II: Two Extrinsic Parameter, One Intrinsic Parameter}
\label{model2:sec}
In \S\ref{model:sec} we show that the Nearby Supernova Factory data analyzed with an agnostic model with
...
absolute magnitudes through global coefficients $\delta$.  Note that $D$ is a latent supernova characteristic whose existence is inferred from
its effects on broadband magnitudes.
Although
this term is introduced through absolute rather than observed magnitudes,
...
The newly fitted $\gamma$ parameters have an almost identical association with the Fitzpatrick dust law, in that now
\begin{equation}
\begin{pmatrix}
 \frac{\gamma^0_X}{\gamma^0_1-\gamma^0_2} \\
\frac{\gamma^1_X}{\gamma^1_1-\gamma^1_2} 
...
\vec{\epsilon},
\end{equation}
where the residual vectors $\vec{\epsilon}$ are perpendicular to the Fitzpatrick plane and
contribute similar 2\% and 5\% to the total  length of the vector.
...
\hline
\end{tabular}
\caption{68\% credible intervals for the Global Fit Parameters of the Intrinsic-Extrinsic Model\label{global2:tab}}
\end{table}
...
with the expectation of dust considering the distribution of supernovae within galaxies and the distribution of galaxy orientations with respect to the observer \citep{1998ApJ...502..177H}.
...
in Figure~\ref{deltacorner:fig}. The left plot shows the $\delta$ posteriors, which are bimodal due to the sign-flip degeneracy.  The probability
of the null result $\delta_X=0$ for all $X$ is $<98\%$, as determined through
the population of its bin in a 5-dimensional histogram.
The right plot shows values of $\delta_X/\delta_I-1$ that  have significant non-zero values.   An interesting feature
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[height=2.5in]{output15/delta_corner.pdf}
      \includegraphics[height=2.5in]{output15/deltaratio.pdf}
   \caption{Significance of the non-zero values of $\delta$.  Left: Posterior contours for $\delta$.  The bimodality is due to the degenerate solution due to sign change.
   Right: $\delta_X/\delta_I-1$, which is insensitive to the sign-flip degeneracy.
...
\begin{pmatrix}
\begin{array}{rrrrrrr}
1.000 & 0.007 & -0.059 & -0.046 & 0.111 & 0.010 & -0.033 \\
0.007 & 1.000 & 0.114 & -0.256 & -0.072 & -0.002 & 0.052 \\
-0.059 & 0.114 & 1.000 & -0.142 & -0.171 & -0.027 & -0.016 \\
-0.046 & -0.256 & -0.142 & 1.000 & 0.039 & 0.028 & 0.014 \\
0.111 & -0.072 & -0.171 & 0.039 & 1.000 & 0.056 & 0.028 \\
0.010 & -0.002 & -0.027 & 0.028 & 0.056 & 1.000 & -0.004 \\
-0.033 & 0.052 & -0.016 & 0.014 & 0.028 & -0.004 & 1.000
\end{array}
\end{pmatrix}.
\end{equation}
\color{red}
The intrinsic parameter $A_{\delta I}$ is uncorrelated with the all the other parameters, including the parameters attributed
to extrinsic dust.
...
with $R^F_V=2.43$.  This model  does an excellent job in
describing SN~2014J, a supernova with extreme colors not used in the training.  
...
colors in a manner that is distinct from  the expectations from dust.
...
\citep{2010A&A...523A...7G}, showing that our model exhibits no loss in its ability to standardize
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5in]{output15/residualx1.pdf}
...
To explore this possibility we plot in Figure~\ref{childress:fig} our intrinsic parameter  $A_{\delta I}$  versus host mass 
\citep{2016rigault}, and their ideograms for two samples separated by a host mass of  $10 \log{(M/M_\sun)}$
...
not significant; we calculate the average $A_{\delta I}$
for low- (high)-mass hosts to be
$   0.0042 \pm    0.0014$
($   0.0031 \pm    0.0017$). 
...
s     \includegraphics[width=2.8in]{output15/rigault2.pdf}
      ...
      less than and greater than  $10 \log{(M/M_\sun)}$.
