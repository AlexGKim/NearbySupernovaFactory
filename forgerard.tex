...
\label{model2:sec}
Analysis of the data using the model presented in \S\ref{model:sec} draws out parameter values consistent
with two-parameter dust models.  We now consider an extension to our model that accommodates an additional
color parameter.
...
\label{dust:eqn}
\end{equation}
In all aspects it is identical to the previous model except for the addition of the term $\delta D$ toward the mean
absolute magnitudes.
Each supernova has an intrinsic $D$ parameter that influences absolute magnitudes with strength $\delta$. 
Although
this term is introduced through the absolute rather than observed magnitudes,
practically identical results are obtained when it is placed alongside the extrinsic $\gamma k$ terms.  Despite this
degeneracy, this term is referred to as ``intrinsic''.
...
the $\gamma k$ terms (i.e.\ dust); practically it is difficult to break the degeneracies between $\delta D$  
and $\gamma k$ in the fit. 
...
results of the 2-parameter extrinsic model given in Table~\ref{global:tab} shows that common parameters
...
We find that the newly fitted $\delta$ parameters have almost identical association with the Fitzpatrick dust law, in that
\begin{equation}
\begin{pmatrix}
 \frac{\gamma^0_X}{\gamma^0_1-\gamma^0_2} \\
\frac{\gamma^1_X}{\gamma^1_1-\gamma^1_2} 
...
The distribution of both sources of color excess $E(B-V)$ and
...
%-----
mag respectively.
The range of $B-V$ colors is $\sim 8$ times larger for the external contribution. 
Both distributions are non-Gaussian, that of $E(B-V)$ has a sharp rise in the blue and an extended tail in the red, consistent
...
in Figure~\ref{deltacorner:fig}. The left plot shows the $\delta$ posteriors, which is bimodal due to the parity symmetry.  The probability
of the null result $\delta_X=0$ for all $X$, lies on a
$>98\%$-contour.  The right plot shows values of $\delta_X/\delta_I-1$, which nulls out the parity symmetry and has significant non-zero values. 

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{output15/delta_corner.pdf}
      \includegraphics[width=2.8in]{output15/deltaratio.pdf}
   ...
   Right: $\delta_X/\delta_I-1$, which as a ratio is insensitive to the degeneracy.
...
We model SNe~Ia broadband optical peak magnitudes allowing for correlations with spectral features at peak and
...
describing SN~2014J, a supernova not used in the training that has atypical colors.  An extended version
of the model retains the features of the first model, but now with strong evidence for a new parameter related to supernova
colors.
...
In fact, light-curve shape is not a strong predictor of supernova magnitude relative to the spectral features: only $\zeta_R/\zeta_V$
has a non-zero value at $>2\sigma$.
The parameters that describe intrinsic color $\delta$ remain significantly non-zero.
...
s     \includegraphics[width=2.8in]{output15/rigault2.pdf}
      \caption{\textcolor{red}{Intrinsic parameter $E_\gamma(B-V)$  versus host mass.  Hosts too faint for a mass measurement
