...
Our magnitude residuals  given in Eqn.~\ref{mag_cov:eqn} are comparable to those of SALT2
\citep[Fig.\ 8 of][]{2010A&A...523A...7G}, showing no obvious disadvantage from not using $X_1$.
Figure~\ref{x1res:fig} shows the difference between observed and model-predicted colors,
as a function of $X_1$.  For all colors, there appear non-linear correlations between $X_1$ and color residuals,
with a negative slope for $X_1<0$ and positive slope for $X_1>0$.
The SN~Ia time evolution does convey information not captured by the peak-magnitude spectral features used in
the current analysis, and  a linear model may be incapable of doing so.
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
...
The posteriors of $\gamma$ are fit to the dust model of \citet{1999PASP..111...63F}, giving
$R^F_V=3.06\pm 0.18$;  Figure~\ref{fitz:fig} shows the   68\% credible intervals of $\gamma$ and the
result of the fit.  Note that  unlike the $R_V$ posteriors, 
the posteriors of $\gamma$ are well-approximated by a normal distribution, which is used for the fit.
\color{black}
...
If $D = D_0$, the $R_V = \gamma_1/(\gamma_1-\gamma_2)$ of the extrinsic contribution is recovered:
this is not the case when $D \ne D_0$.  Again considering a set of SNe~Ia  with parameters set to the median fit values of the SNfactory sample,
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=3.2in]{output11/rveff4.pdf}
      ...
      $k_0=0$, $\delta_0=0$ supernove,  for  a set of SNe~Ia  with parameters set to the median fit values of the SNfactory sample.}
   \label{rv4:fig}}
\end{figure}
