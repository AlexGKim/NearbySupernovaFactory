...
A recent description of the current status of the data is given in \citet{2015ApJ...815...58F}.
...
$\Lambda$CDM cosmology with $\Omega_M = 0.28$ (but with an arbitrarily selected
$H_0$ since we are interested in relative magnitudes).
...
closest to maximum is used \citep{chotard:thesis}. [This is what Nico did in his thesis, is this true of the data Manu gave me?]
...
(In this article these parameters are equivalently subscripted with $01234$ or $UBVRI$.)
...
As the model cannot break the degeneracy, we do so
...
deviations denoted by $\sigma_i = \sqrt{C_{c,ii}}$ with a  Cauchy distribution prior with location
...
Empirically, the confidence regions are localized and unimodal as can be seen in  Figure~\ref{global:fig}.  We find no evidence that
   ...
   \includegraphics[width=2.5in]{output11/coeff2.pdf} 
      \includegraphics[width=2.5in]{output11/coeff3.pdf} 
         \includegraphics[width=2.5in]{output11/coeff4.pdf} 
            \caption{Posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\gamma$, $\delta$, and $\sigma$ in each of the 5 bands.   \label{global:fig}}
\end{figure}


For each of the five filters, the 68\% credible equal-tailed intervals for the global parameters $\alpha$, $\beta$, $\eta$, $R$, $R_\delta$, and $\sigma$
...
values of zero signify no color changes.
...
Figure~\ref{ccm:fig},
along with the expectation of the \citet{1999PASP..111...63F} dust model.
The $R_X$ are consistent with a dust-model parameter  $R^F_V=2.5$.  The 
$R_X$ parameters are correlated: the alternative parameterization
...
Therefore, our model does not specify a unique $R_{B,eff} = A_{B, eff} /E_{eff}(B-V)$.

We plot $E_{eff}(B-V) + const$ versus
$A_{B, eff}$ for the SNf sample in Figure~\ref{rveff:fig}.
Overplotted is the expected extinction due to a pure extrinsic extinction with $\gamma_1$ slope.
The constant added to the effective color excess
corresponds to specifying a specific color as having zero extinction, and is chosen for qualitatively to form a ridge line 
...
The right plot of Figure~\ref{rveff:fig} shows (relatively) how much more extinction is
...
A linear fit between $E_{eff}(B-V)$  and $\Delta A_B$ gives a slope of $-0.33$.
Although our model does not fix $R_{B,eff}$, it does show that it decreases on average with increasing effective color excess.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{output11/rveff.pdf}
   \includegraphics[width=2.8in]{output11/rveffres.pdf}
...
most strongly correlated with $EW_{Si}$, while the internal $E_\delta(B-V)$  has no correlation  with internal parameters stronger than
...
is also plotted.  Our model has significantly smaller deviations from the data.

In our model, the observed color excess is attributed to   extrinsic
...
comparable to $E(B-V)$ from \citet{2014A&A...568A..22B}.
A linear fit for the relationship between color and Hubble residual gives a slope of $-0.67$.
...
non-zero $\eta$ values at  $\gtrsim 2\sigma$.  We  anticipate that in the previous analysis, equivalent widths of CaII and
