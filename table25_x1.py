...
for i in xrange(10,25):
    H,edges  = numpy.histogramdd(fit['rho1'],bins=i)
...
print "rv kinda"
crap=[]
for i in xrange(fit['k'].shape[0]):
    x=fit['k'][i,:]*(fit['gamma'][i,1]-fit['gamma'][i,2]) + fit['k1'][i,:]*(fit['gamma1'][i,1]-fit['gamma1'][i,2])
    y=fit['k'][i,:]*fit['gamma'][i,2] + fit['k1'][i,:]*fit['gamma1'][i,2] 
    dum=scipy.stats.linregress(x, y)
    crap.append(numpy.array([dum[0], dum[1]]))
crap = numpy.array(crap)
print crap.shape
print numpy.percentile(crap[:,0],(50,50-34,50+34))
wefwe
...
pars_n = ['\\alpha_X','{\\alpha_X/\\alpha_V-1}','\\beta_X','{\\beta_X/\\beta_V-1}',\
  '\\eta_X','{\\eta_X/\\eta_V-1}', '\\zeta_X','{\\zeta_X/\\zeta_V-1}', '\\gamma^0_X', '{\\gamma^0_X/\gamma^0_V-1}', '\\gamma^1_X','{\\gamma^1_X/\\gamma^1_V-1}',\
   '\\delta_X','{{\\delta_X/\\delta_U-1}}','\\sigma_X']
...
for p,pn, s in zip(pars,pars_n,sigfig):
    ...
    for i in xrange(5):
        ...
        if pn[0] == 'R':
            ...
            dum = numpy.percentile(fit[p][:,i]/fit[p][:,0]-1,(50,50-34,50+34)) 
        elif pn[0] == '{':
            dum = numpy.percentile(fit[p][:,i]/fit[p][:,2]-1,(50,50-34,50+34)) 
        else :
            dum = numpy.percentile(fit[p][:,i],(50,50-34,50+34))            
...
mega = numpy.array([fit['Delta'],fit['EW'][:,:,0],fit['EW'][:,:,1],fit['sivel'], \
    ((fit['gamma'][:,1] - fit['gamma'][:,2])[:,None]*fit['k']),((fit['gamma1'][:,1] - fit['gamma1'][:,2])[:,None]*fit['k1']),(fit['rho1'][:,0][:,None]*fit['R'])])
