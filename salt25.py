...
# wefe
A2= salt_band.A_X(c=(c+0.01))
dAdc = (A2 - A1)/0.01

print '{0[0]:6.2f}, {0[1]:6.2f}, {0[2]:6.2f}, {0[3]:6.2f}, {0[4]:6.2f}'.format(dAdc)

g0, cmin, cmax = numpy.percentile(fit['gamma']/((fit['gamma'][:,1]-fit['gamma'][:,2])[:,None]),(50,50-34,50+34),axis=0)
g1, cmin, cmax = numpy.percentile(fit['gamma1']/((fit['gamma1'][:,1]-fit['gamma1'][:,2])[:,None]),(50,50-34,50+34),axis=0)
...
norm_dAdAv = numpy.dot(g0, g0)
norm_dAdebv = numpy.dot(g1, g1)
cross = numpy.dot(g0, g1)

a = numpy.array([[norm_dAdAv,cross],[cross,norm_dAdebv]])

y = numpy.array([numpy.dot(dAdc,g0), numpy.dot(dAdc,g1)])
ans = numpy.linalg.solve(a,y)
ans = dAdc-ans[1]*g1 - ans[0]*g0

print (numpy.linalg.norm(ans)/numpy.linalg.norm(dAdc))**2
wefe
