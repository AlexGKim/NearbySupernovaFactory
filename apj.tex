...
To visualize the above result, Figure~\ref{plane:fig} shows two perspectives (left and right columns) of $UVI$- (top row) and $BVR$- (bottom row) spaces
...
A prior that $k_0$ and  $k_1$ be uncorrelated was enforced to break degeneracies in our linear models, and led to the specification of the directions of
...
\end{equation}
with the parameters $\kappa_1$ and $\kappa_2$ providing the freedom of choice in the directions of the two basis vectors.  In the analysis,
the arbitrarity is broken and the $\kappa$'s specified, by the  prior of  
zero covariance between $(A^F_V - \kappa_2 E^F(B-V))$ and  $(-\kappa_1 A^F_V + E^F(B-V))$.
The preferred values for $\kappa$ from the linear model in \S\ref{model:sec} are shown qualitatively in
...
is aligned with $(\kappa_2 a(X) + b(X))$ for $X=UVI$.  Slightly shifted values of $\kappa_1=1/2.6$, $\kappa_2=-6.1$ apply for  $X=BVR$.

An intuitive understanding of $\kappa_1$ can be obtained by considering what would happen if
all supernovae were to experience dust with the same $R^F$.   The zero-covariance
prior would be satisfied by  $\kappa_1=1/R^F$ and any arbitrary $\kappa_2$.
The combination  $\gamma^0_V k_0$ would correspond to  $A^F_V$.  Intrinsically $k_1=0$  and $\gamma^1$ would be unconstrained.
A slight relaxation away from the constant $R^F$ assumption would result in non-zero $k_1$ and a $\gamma^1$ in a direction that satisfies
the prior condition, as parameterized by $\kappa_2$.
...
in standard models of Milky Way dust.  There do remain some small inconsistencies between our numbers and expectations from dust models.
\color{black}
