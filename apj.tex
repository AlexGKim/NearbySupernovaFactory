...
\color{red}
in the $U$ and $B$ bands.  The   $<1\sigma$ changes in $\pmb{\gamma}$ lead to a slight increase in the 
 \citetalias{1999PASP..111...63F}
$R^F$-parameter 68\% credible interval
from $2.26$--$2.63$ to $2.31$--$2.69$.
\color{black}
...
the point $\delta_X=0$  lies on the 85\%-ile contour.
Model~II has a  $\pmb{\delta} \rightarrow -\pmb{\delta}$, $D \rightarrow -D$ that results in the bimodal peaks in the distribution.
...
The modes of $\pmb{\delta}$ are at $\mp 1.12$, $\mp 0.89$, $\mp 0.36$,  $\mp0.16$, $\pm0.21$ in $UBVRI$ respectively.
The values of $\delta_X/\delta_U-1$, statistics insensitive to the scaling (including the sign) degeneracy, are shown in Figure~\ref{deltaratio:fig}.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   Right: The same posteriors after imposing that  the one supernova with  the $D$ of the one supernova with the highest signal-to-noise be positive.
...
First, while the degeneracies in the model could allow a single physical color effect to be
distributed between both intrinsic and extrinsic parameters, from the correlations
there is no evidence
that this occurs.
Second, the intrinsic term cannot be attributed to non-linearity that is inherent to the broad-band extinction in dust models, since
in that case a correlation between the  $k$'s and $D$ would be expected.
...
These results are similar to those of Model I in Eqs.\ \ref{trans_I:eqn}--\ref{res_I:eqn}.
\color{red}
...
For one set of tests, we realize 50 random realizations of data with input  parameters equal to the median values found in
\S\ref{model:sec} and the same noise as our data.  The input values of $\pmb{\delta}$ are set to zero.
In Figure~\ref{pvalues_delta2:fig}, the squared amplitudes  $\pmb{\delta}^2$
from the null realizations are compared with that of our fit to the data.  Depending on the choice either the  $-1$, 0, or $1$-sigma
value, the data value is at the 90--98\%-ile.  Contrast these $p$-values with the 85\%-ile contour on which $\pmb{\delta}=0$ lies
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
  ...
  \includegraphics[width=2.9in]{output25_sim_null/pvalues_relperp.pdf} 
            \caption{ The squared length $\pmb{\delta}^2$ (left) and relative length perpendicular to the plane defined by $\pmb{\gamma}$ $\delta_\perp^2/\delta^2$ for 50 independent simulated data sets with input $\delta=0$.  The fits
...
The significance of the close projection of $\pmb{\delta}$ onto the plane defined by the $\pmb{\gamma}$'s is shown
in  Figure~\ref{pvalues_delta2:fig}, which plots the relative amplitude projected out of the plane $\delta_\perp^2/\delta^2$ for the null realizations
and our data.  Our data has smaller value than all the realizations for $-1$, 0, or $1$-sigma values.  This alleviates
the concern that our model and sampling spuriously produce $\pmb{\delta}$ closely aligned with the $\pmb{\gamma}$--plane, i.e.\
the small  $\delta_\perp^2/\delta^2$ of our data is not expected to be due to noise at 98\%.
...
biases in the best fit value.  The stack of   $\pmb{\delta}$ is shown in Figure~\ref{simnulldelta:fig}.  The correlations between the parameters
