...
Empirically, the confidence regions are localized and unimodal as is seen in  Figures~\ref{global1:fig} -- \ref{global5:fig}.  We find no evidence that
...
We now compare our model and its color-coefficient parameters $\gamma^0$ and $\gamma^1$ with the expectations of  dust laws.
...
\label{f99:eqn}
\end{equation}
approximately describes dust laws.
For the case of
$R^F=2.5$ and $A^F_V=0.1$,
and the measurement of the updated
\citet{2007ApJ...663.1187H} SN~Ia template at $B$-band peak, the \citetalias{1999PASP..111...63F} model
gives
...
Over the ranges of
 $R^F$ and $A^F_V$,
and the wavelengths under consideration,  the values of the components  vary by $<5$\%.
...
%----
can be transformed to a basis composed of the $a(X)$ and $b(X)$ vectors, and a residual vectors perpendicular to $a$ and $b$.
The result is
\begin{equation}
\begin{pmatrix}
...
To visualize the above result, Figure~\ref{plane:fig} shows two perspectives in $UVI$-space
...
The left plot shows that the four vectors point in different directions, while the right plot shows that
the planes defined by our model (solid lines) and the  \citetalias{1999PASP..111...63F} model (dashed lines) are nearly coplanar.

The specific directions of $\frac{\gamma^0_X}{\gamma^0_B-\gamma^0_V}$ and
$\frac{\gamma^1_X}{\gamma^1_B-\gamma^1_V}$ within the plane they define are not arbitrary, but are determined by the prior that
$k_0$ and  $k_1$ are uncorrelated. 
In order to understand these directions in the context of the dust model,
we
rewrite the \citetalias{1999PASP..111...63F} model in Eqn.~\ref{f99:eqn} as
\begin{equation}
A_X =  A_V^F\left(a(X)+\frac{b(X)}{r} \right) +  \left( E^F(B-V) - \frac{ A_V^F}{r}\right)b(X).
\end{equation}
Assigning $r=2.4$, we find that the combination  $\left(a(X) + \frac{b(X)}{2.4}\right)$ is
...
as shown in the dot-dashed and dotted lines of  Figure~\ref{plane:fig}.  This implies a correspondence of $k_0$ with $A^F_V$,
and $k_1$ with $  \left( E^F(B-V) - \frac{ A_V^F}{2.4}\right)$.
Our prior of a non-correlation between $k_0$ and $k_1$ has lead to an $r \sim 2.4$  that optimizes the
non-correlation between $A^F_V$ and $  \left( E^F(B-V) - \frac{ A_V^F}{r}\right)$.
...
Given the association of  our parameter $\gamma^0_2 k_0$ with the $A^F_V$ of an $R^F \sim 2.4$ dust model ,
the former's distribution can be compared to the expectation for the latter.
Figure~\ref{k0_med:fig} shows the histogram of
median values of $\gamma^0_2 k_0$.
...
Figure~\ref{k0_med:fig} also shows the distribution of $\gamma^1_2 k_1$, which is associated with $\left( E^F(B-V) - \frac{ A_V^F}{2.4}\right)$, i.e.\
extinction corrections due to deviations away from the $R=2.4$ value.   The distribution has a long
negative tail and is peaked at its positive edge, and by construction has a mean of zero.
The implication is that while the bulk of supernovae have positive values associated with $R>2.4$, there is a tail
of negative values associated with $R<2.4$.  The plot of   $\gamma^0_2 k_0$ versus $\gamma^1_2 k_1$  shown in Figure~\ref{kk:fig} shows that
the positive edge in $\gamma^1_2 k_1$ persists over the full range of  $\gamma^0_2 k_0$. On the other hand, lower values of $\gamma^1_2 k_1$
occur at larger values of  $\gamma^0_2 k_0$.
...
median values of Left: $\gamma^0_2 k_0$, which is associated with the $A^F_V$ of an $R^F \sim 2.4$ dust model; Right: $\gamma^1_2 k_1$,
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output11/kk.pdf}
      \caption{ $\gamma^0_2 k_0$ versus $\gamma^1_2 k_1$.
   \label{kk:fig}}
\end{figure}
%
...
$0\le A_V \le 1.8$, $1 \le R \le 8$ with a third-order polynomial in $A_V$ and $E(B-V)$.
% The residuals between the model and
the approximation for $A_V=1.8$ are  shown in Figure~\ref{dfitz:fig}. 
The amplitude residuals in $B$ through
...
each supernova, with a uniform prior $0\le A_V \le 1.8$, $1 \le R \le 8$.
(Note that for $R<2.2$, the model extrapolates beyond the range of the training set upon which it was trained.) 
...
each supernova, for the three $R$ distributions.
...
each supernova, for the analyses assuming constant $R$, $\ln{R}$ from a Normal distribution, and $R$ from a uniform distribution.
To contrast, the corresponding $\gamma_0 k_0$ medians from our 2-parameter model is also shown.  The means of the measurement
...
The distributions of $A_V$ and $R$ are not as one would expect for an $R=3.1$ Milky Way dust.  While the $R=const$ case gives $R \sim 3.5$, its
$A_V$ distribution  has a slower rise from zero and a non-zero mode of $\sim 0.4$ mag, inconsistent with the expectation of a large fraction of supernovae
...
The broadband nature of our supernova model does not allow a precise specification for the prediction of dust models, we therefore
refrain from making absolute conclusions based on our results.  We do emphasize the sensitivity of our results when using the total-to-selective extinction
$R$ parameter.  
