...
\citet{2017ApJ...842...93M} present an instructive simulation, producing magnitudes and colors of a set of
supernovae 
with an intrinsic source of color variability having relatively smaller value of $\beta$
and a realistic distribution of dust extinction having a relatively larger $\beta$ value.  The expectation is for
supernovae encountering no interstellar dust  to tend to be bluer and exhibit the intrinsic $\beta$ slope for the color-magnitude
relation.  Supernovae encountering thick dust tend to be redder and exhibit the larger dust $\beta$ slope.
The expectation is borne out in their Figure 3, which shows 
observed color versus light-curve shape corrected magnitudes, and is seen in their data Figure 6.
The distinctive expectations manifest in the plot  for the above model allow
the authors to use  information inherent in the distributions to make distinct measurements of intrinsic and dust slopes (referred
therein as $\beta$ using the terminology of SALT2),
despite being
restricted to using one color observable that is  affected by two sources of extinction.

Our Model~II has for each supernova three color parameters that can contain information on more
than one source of color variablility. This allows us to construct from our fit results per-supernova magnitude corrections as a function
of color, to produce a model-based figure analogous to Figure~3 of \citet{2017ApJ...842...93M}.
Such a result is shown in Figure~\ref{speccorr:fig}, which plots
$U$-band magnitude corrections from the three color parameters $\gamma^0_{U} k_0 +\gamma^1_{U} k_1 + \delta_U D$
as a function of $E_o(B-V)$, where the expected color accounts for spectral-feature corrections.
The shallower slope at blue color transitioning to a larger slope at red color 
is correspondence with the expectation of a mixed intrinsic--dust model of the form considered by  \citet{2017ApJ...842...93M}.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{output25/ccorr2e.pdf}
      \caption{
      \color{red}
      The $U$-band magnitude corrections from the three color parameters $\gamma^0_{U} k_0 +\gamma^1_{U} k_1 + \delta_U D$
are plotted as a function of $E_o(B-V)$, where the expected color accounts for spectral-feature corrections.  The left plot shows the full
