...
which a linear fit gives a slope of $R_V=2.77^{+0.08}_{-0.09}$.  The fit also gives the zeropoint, which we use to 
estimate $R_V=(A_V-zeropoint)/E(B-V)$ per supernova, shown in Figure~\ref{avebv:fig}.
Extremely blue supernovae ($E(B-V) <-0.05$ on the plot) have an average $R_V=1.88 \pm   0.13$,
whereas the extreme red objects ($E(B-V) >0.1$) have $R_V=  2.96 \pm   0.11$, indicating that different physical mechanisms
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Left: $A_V$ versus $E(B-V)$ and Right: $R_V=A_V/(E(B-V)-zeropoint$\
...
\color{red}
Plotted in Figure~\ref{avebv:fig} are the expectations for measured $A(V)$, $E(B-V)$, and $R_V$ for $R^F_V=1.9, 2.4, 3.1$
...
non-linear, and $R_V$ is not constant, for a fixed value of $R^F_V$.
\color{black}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Two different perspectives in $UVI$-space of the vectors $\gamma^i_X/(\gamma^i_B-\gamma^i_V)$ of our model (solid lines),
