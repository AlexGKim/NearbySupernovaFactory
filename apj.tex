...
Given the association of  our parameter $\gamma^0_V k_0$ with the $A^F_V$ of an $R^F \sim 2.4$ dust model ,
...
Figure~\ref{k0_med:fig} also shows the distribution of $\gamma^1_V k_1$, which is associated with $\left( E^F(B-V) - \frac{ A_V^F}{2.4}\right)$, i.e.\
...
of negative values associated with $R<2.4$.  The plot of   $\gamma^0_V k_0$ versus $\gamma^1_V k_1$  shown in Figure~\ref{kk:fig} shows that
...
SN~Ia, independent of spectral-feature parameters.  This choice contrasts with the models in the previous sections, which do not specify the unextincted
supernova.
The SALT2 SED determines the values of $A_X$ and the
intrinsic colors. 

This ansatz is  inconsistent with the results of the linear models evaluated with SNf data presented in previous sections:  the colors of a canonical supernova are not accessible by a SALT2 SED experiencing  \citetalias{1999PASP..111...63F} dust.
Relative to SALT2, the canonical supernova with colors given by the best-fit $c_X$'s in \S\ref{model:sec} has the broad-band color excesses $E(X-V)$ shown in
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{output18/comptolinear.pdf}
    ...
    relative to SALT2.  Overplotted are possible $E(X-V)$ for a fixed $A_V$ and a range of $R$,
allowed by \citetalias{1999PASP..111...63F} dust.
Right:    \label{model1118:fig}}
\end{figure}
...
and
\ref{dust:eqn} in \S\ref{model:sec}, except that the colors of a zero-extinction supernova (parameterized by differences in $c_X$) are fixed to those of the SALT2 model,
...
The presentation of our results from evaluating this model with SNf data
begins with further evidence of the inconsistency between the ansatz and the models of the previous section.
The current analysis sets the magnitude of an $A_V=0$ supernova, so now  magnitudes in addition to colors of the canonical
supernova in \S\ref{model:sec} can be compared relative to expectations of the dust model.  The magnitude difference
is the difference between the $c_X$ parameters of the two analyses, denoted as $\Delta c_X$.
...
The analysis results for the dust parameters are shown in Figure~\ref{AVRV:fig}.  The left plot shows the histogram for the median $A_V$ determined for
each supernova. The
$A_V$ distribution  has a slow rise from zero,  compared to the equivalent $\gamma^0_V k_0$
from the linear model of \S\ref{model:sec} that has the sharp rise expected from of a large fraction of supernovae
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{output18/RVs_hist.pdf}
    \caption{Left: histogram for the median $A_V$ determined for
