...
The $F$ superscript distinguishes parameters of the \citetalias{1999PASP..111...63F} model.
Over the ranges of
 $R^F$ and $A^F_V$,
and the wavelengths under consideration,  the values of the elements of $a$ and $b$ change but only by $<5$\%;
\color{red}
for $0\le A^F_V\le 1$ and $2 \le R^F \le 3.5$, the largest error in Eqn.~\ref{f99:eqn} has amplitude $<0.008$ mag.
\color{black}
...
\end{equation}
where $E_{\gamma^i}(B-V) \equiv  (\gamma^i_B-\gamma^i_V)k_i$.
...
Our model $\gamma$-based vectors can be written in terms of the 
...
the allowed variations in $UBVRI$ allowed by the \citetalias{1999PASP..111...63F} model and our best-fit model are confined to near-parallel 2-dimensional
planes within
...
and $\mathbf{a}$, $\mathbf{b}$ of the \citetalias{1999PASP..111...63F} model (dashed lines).  All $\pmb{\gamma}$ vectors are chosen to intersect the origin.
...
vectors span near-parallel planes in the 5-dimensional color space, and hence each pair can be almost entirely expressed in  terms of
the other pair.
...
%When fixing the origin of $\pmb{\gamma}$ to zero, the $\pmb{\gamma}$ vectors are found to be coplanar with $\mathbf{a}$ and $\mathbf{b}$.
%While the  $\gamma$ vectors define a plane,  is unspecified.
%The linear model does not specify zero dust so the origin of the $\gamma$'s is unspecified.
...
%The dust model expressed in Eqn.~\ref{f99:eqn} can be reparameterized as
%\begin{equation}
%A_X = A_V \left(a(X)  +  \frac{b(X)}{R}\right),
%\label{f99:eqn}
%\end{equation}
%where $R=A_V/E(B-V)$.
\color{purple}
In principle, our linear model could accommodate for $\gamma$ any two independent vectors that span
the plane.  We begin discussion on the specific choice of $\gamma$ returned by the fit within the context of the \citetalias{1999PASP..111...63F}  dust model. 
While
...
\end{equation}
with the new parameters $A^F_V - \kappa_2 E^F(B-V)$ and $-\kappa_1 A^F_V + E^F(B-V)$  proportional to  linear combinations of the original parameters.

Our model includes a prior that the per-supernova parameters $k_0$ and  $k_1$ associated with $\gamma$ are uncorrelated.
...
to the non-correlation of   $A^F_V - \kappa_2 E^F(B-V)$ and $-\kappa_1 A^F_V + E^F(B-V)$ in the supernova sample.
...
Let us introduce a new per-supernova vector $\delta R$ such that $R^F = R^F_{eff}(1+  \delta R)$.
Variations in the value of $R^F$ (and hence $\delta R$) lead 
the prior to impose
\begin{equation}
...
deviations from an effective $R^F$ and
...
measurement uncertainty are the only sources of variation in magnitudes.  We stack the results of the 10 simulations which yield a median
$R^F_{eff}$ of 2.53, which is close to
 the input $R^F=2.5$.
...
with the expectation of dust based on the spatial distribution of supernovae within galaxies and the distribution of galaxy orientations with respect to the observer \citep{1998ApJ...502..177H}.  This distribution is given by the data, in that it is independent of any knowledge or prior
for the distribution of dust extinction the SN population is expected to suffer. 

Figure~\ref{k0_med:fig} also shows the distribution of $\gamma^1_V k_1$
relative to that of an arbitrary supernova  $\gamma^1_V k_1|_0$, which can be attributed to 
extinction corrections due to deviations away from the canonical $R^F_{eff}$ value.   The distribution has a long
negative tail and is peaked at its positive edge. Recall that our model constraints force 
the distribution of $\gamma^1_V k_1$ has a mean of zero.
...
\color{purple}
The model parameters can be transformed into  the 
...
Differences in the color predictions between our model and the pure dust model are
apparent,
particularly in the $B-V$ and  $R-V$ colors.  Our model has shallower decline in color excess with increasing wavelength.
(Refer to  Eqn.~\ref{res_I:eqn} to see the deviation in magnitude of our model away from the dust model.)
...
\end{pmatrix},
\end{equation}
which imply $R^F= 0.59^{+ 0.53}_{-0.48}$.
...
gives $k_0$ and $k_1$ that qualitatively similar to  those from the fit to Model~I but with much larger uncertainties.
...
The motivation of our analyses was to standardize supernovae based on spectral features and colors around peak brightness.
...
has the largest significance at at $1\sigma$.
...
The difference between high-mass hosts is
not significant; we calculate  average
%---
$\langle A_{\delta I} \rangle=     -0.0002 \pm    0.0089 $,
$\langle A_{\delta I} \rangle= 0.0025 \pm   0.0073 $ 
...
each Monte Carlo link.  The resulting distribution of the two-tailed p-value (a high value meaning that we cannot
reject the hypothesis of the samples are the same) over all chains is shown in Figure~\ref{childress:fig}. The distribution is broad
over the range from 0 to 1.  We are thus unable to distinguish the $A_{\delta I}$ distributions from low- and high-mass samples.
...
A straightforward explanation for the third color term $\pmb{\delta} D$ is that it describes the absorption due to some form of dust
that does not obey the  \citetalias{1999PASP..111...63F} model.
Supernova light may encounter two
dusts, first a 
circumstellar dust that can produce lower visual-to-selective extinction
...
The projection of  $\pmb{\delta}$ onto the plane expected by dust is significant at the $>98$\% level.  However, we
lack the signal-to-noise to show consistency with this explanation.
Our best-fit  $\delta_X$ do not share the same sign, as expected for dust.
...
An alternative explanation for the $\pmb{\delta} D$ term is that it captures an intrinsic dust parameter that is independent of $R^F$.
Supernova light encounters one dust with a degree of freedom beyond that modeled by \citetalias{1999PASP..111...63F}.
