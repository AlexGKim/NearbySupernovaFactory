...
$>95$\% confidence;
%-----
this new parameter is responsible for supernova color diversity
...
but this choice does not specify the $k_0^0$, $k_1^0$ corresponding to supernovae experiencing no extrinsic effects.  The true extrinsic
extinction is
...
Our model as analyzed cannot directly determine the above extrinsic properties per supernova.
...
$$
Making the assumption that $R^T_V$ is the same for all supernovae (shown later to be approximately true for dust laws),
it can be determined as the slope of the relation between $A_X$ and $E(B-V)$.
\color{black}
Figure~\ref{avebv:fig} shows the relationship between $A_V$ and $E(B-V)$, from
which a linear fit gives a slope of
%----- fitz11.py
$R_V=2.77 \pm 0.08$.
%----
\color{blue}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   for specific choices of $R^F$.  Note that these lines are not straight on the left plot.
   \color{black}
   \label{avebv:fig}}
\end{figure}
%\end{comment}

We now compare our model and its color-parameters $k_0$ and $k_1$ with the expectations of  dust laws,
in contrast to  empirical properties derived from our model.
...
\color{red}
 $R^F$ and $A^F_V$,
\color{black}
and wavelengths under consideration,  the values of the components  vary by $<5$\%.
...
\color{red}
Plotted in Figure~\ref{avebv:fig} are the expectations for measured $A(V)$, $E(B-V)$ for $R^F=1.1, 1.9, 2.5, 3.1$
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4.5in]{output23/delta_corner.pdf}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.5in]{output23/deltaratio.pdf}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.5in]{output23/perobject_corner.pdf} 
   \caption{
   \color{red}
...
\section{Supernova Model 0: Fitzpatrick Dust}
\label{model0:sec}
We analyze our data with a model that incorporates attenuation due to host-galaxy dust.
...
specifies the wavelength-dependent  attenuation  notated by the function $A/E(B-V)$.  The second, $E(B-V)$, specifies the normalization
of that attenuation.  The dust model is expressed as
\begin{equation}
A(\lambda; E(B-V), R) = E(B-V) \frac{A}{E(B-V)}\left(\lambda; R \right).
\end{equation}
Those two parameters can be combined to define a third dependent parameter $A_V \equiv R E(B-V)$.
...
In our model for broadband magnitudes, the SEDs of supernovae $f(x)$ are not specified.  In its place we use the 
...
$0\le A_V \le 1.8$, $1 \le R \le 8$ with a third-order polynomial in $A_V$ and $E(B-V)$.  The residuals between the model and
the approximation for $A_V=1.8$ are  shown in Figure~\ref{dfitz:fig}.  The amplitude residuals in $B$ through
$I$ are $< 0.001$ mag; in $U$  for $R>1$ the residuals are $<0.004$ mag
and increase to $0.01$ mag as $R$ decreases to $1$.



\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.5in]{output18/dfitz.pdf}
...
Three distinct analyses are run with the  \citetalias{1999PASP..111...63F}  dust model.  In the first, $R$ is held the same for all supernovae (labeled
as $R=const$).  In the second,
the $\ln{R}$ for each supernova is taken to be drawn from a Normal distribution with dispersion $\sigma_{\ln{R}}$ (labeled as $\ln{R} \sim \mathcal{N}$).
In the third, $A_V$ and $R$ are free for
each supernova, with a uniform prior $0\le A_V \le 1.8$, $1 \le R \le 8$ (labeled as $R$-free).

The posteriors for the spectral parameters are similar to the ones already
presented; we therefore focus on the posteriors for the dust parameters, the results of which
are shown in Figure~\ref{AVRV:fig}.  The left plot shows histograms for the median $A_V$ determined for
each supernova, for the analyses assuming constant $R$, $\ln{R}$ from a normal distribution, and $R$ from a uniform distribution $1 \le R \le 8$.
The right plot shows the posteriors for $R$ for the two analyses that constrain the $R$ distribution, and the histogram of
the median $R$ determined for each supernova in the $R$-free analysis.  (The  $\ln{R} \sim \mathcal{N}$-case draws $R$ from
the Normal distributions specified by individual links.)

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{output18/AVs_mode_hist.pdf}
   \includegraphics[width=2.8in]{output18/RVs_hist.pdf}
    \caption{Left: Histograms for the median $A_V$ determined for
each supernova, for the analyses assuming constant $R$, $\ln{R}$ from a Normal distribution, and $R$ from a uniform distribution $1 \le R \le 8$.
Right: Posteriors for $R$ for the two analyses that constrain the $R$ distribution, and the histogram of
the median $R$ determine for each supernova in the $R$-free analysis.
   \label{AVRV:fig}}
\end{figure}

Both the distributions in $A_V$ and $R$ are dependent on the assumptions on the distribution of $R$.
The mode in the $A_V$ histogram shifts from $\sim 0.2$ mag 
for the $R$-free case, to $\sim 0.4$ mag for  $R=const$, and to $\sim 0.6$ for  $\ln{R} \sim \mathcal{N}$.
The  posterior of $R$ is skinnier and biased for $R=const$ relative to the $R$ distributions found by the $R$-free and  $\ln{R} \sim \mathcal{N}$
cases.

The distributions of $A_V$ and $R$ are not as one would expect for an $R=3.1$ Milky Way dust.  While the $R=const$ case give $R \sim 3.5$, its
distribution of $A_V$ has a slower rise from zero and a non-zero mode of $\sim 0.4$ mag, inconsistent with the expectation of a large fraction of supernovae
experiencing no dust, with a tail in $A_V$ for those that do \citep{1998ApJ...502..177H}.
The $\ln{R} \sim \mathcal{N}$ distribution for $A_V$ is shifted and a mode of  $R$
larger that $4$.
While for $R$-free the  $A_V$ distribution mode is centered somewhat closer to zero, the distribution of $R$ is shifted to even higher values.


The Hsiao template does not perfectly represent the underlying spectrum of a supernova.  Consider the SED to be the sum of the template and an offset
\begin{equation}
f(\lambda) = f_0(\lambda) + \delta f(\lambda).
\end{equation}
...
extinction.  Heterogeneity in the true spectrum amongst supernovae yield scatter in those corrections.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.5in]{output23/residualx1.pdf}
