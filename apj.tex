...
\begin{abstract}
\color{orange}
Empirical modeling has led to precise determinations of the peak absolute magnitudes of Type~Ia supernovae (SNe~Ia), making this class
of object an excellent distance indicator.  Improved modeling of SN~Ia absolute magnitudes may account for physical effects that
are expected but not included
in current models, lead to more precise per-supernova distances, and further constrain systematic uncertainties.  In this article, we present the
results of    
\color{black}
...
Of the three extreme outliers, the supernovae with largest and third largest $\Delta$'s have  heliocentric redshifts of 
 $0.0015$ and $0.0085$ respectively, and so have peculiar velocities that can contribute significantly to $\Delta$.
\color{black}
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
...
\color{orange}
Of particular interest in $C_c$ is intrinsic color dispersion.  The intrinsic dispersions in $V$ and colors with respect to $V$ are
\begin{equation}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   between the data and model, with a dotted line at zero shown for reference.
...
\color{orange}
Our model and its fit parameters can be associated with the extinction model of  \citetalias{1999PASP..111...63F} using Eqn.\ref{trans_I:eqn}
and the assumption that  SN~2011fe has zero extinction.  The inferred  \citetalias{1999PASP..111...63F} parameters
are $A^F_V=1.28$ and $E^F(B-V)=0.77$ (which together imply $R^F=1.66$) with covariance
\begin{equation}
\begin{pmatrix}
\begin{array}{rr}
0.007 & -0.004 \\
...
relative to the statistical uncertainties.   Note that some bias is expected: the priors for the distributions of $\Delta$, $k$, and $D$ are inconsistent
with the extracted distributions.
%There is a tendency for the slope parameters to be slightly underestimated, which is an expected consequence of 
...
\color{orange}
for the subset of supernovae whose hosts are given in \citet{2013ApJ...770..108C}.
