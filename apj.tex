...
The global parameters $\pmb{\gamma}^0$, $\pmb{\gamma}^1$  are the slopes that connect the extrinsic-color
...
in our linear model these values are otherwise unconstrained.
The model contains terms like $\pmb{\gamma} k$, which lead to the degeneracy $\pmb{\gamma} \rightarrow a\pmb{\gamma}$, $k \rightarrow a^{-1} k$.
While the physical results are independent of this scaling, we impose a prior on the rms of $k$ to aid the convergence during model fitting.
This prior does not specify the sign of $a$, which leaves a degeneracy.  As will be seen, the signal-to-noise in $\pmb{\gamma}$ is sufficiently
high that our finite MCMC chains do not migrate between the degenerate solutions; for convenience
we impose 
\begin{equation}
...
The combinations $\pmb{\gamma}^0$--$k_0$ and $\pmb{\gamma}^1$--$k_1$ are degenerate with
...
There remain degeneracies in the model.  One is illustrated by the  transformations $\Delta \rightarrow \Delta  + \epsilon_\Delta$,
 $\pmb{\gamma}^0 \rightarrow \pmb{\gamma}^0  + \epsilon_{\pmb{\gamma}^0}$, $\pmb{\gamma}^1 \rightarrow \pmb{\gamma}^1 + \epsilon_{\pmb{\gamma}^1}$under the condition
$$
\epsilon_\Delta  +  \epsilon_{\pmb{\gamma}^0} k_0+  \epsilon_{\pmb{\gamma}^1} k_1=0,
$$
i.e.\ shifts in the global coefficients $\pmb{\gamma}$ can be compensated by shifts in $\Delta$ for each supernova while maintaining 
$\langle \Delta \rangle=0$.
Another degeneracy is illustrated by $k_0 \rightarrow k_0 + (EW_{Ca}-\langle EW_{Ca}\rangle)\epsilon$,
$\alpha \rightarrow \alpha - \pmb{\gamma} \epsilon$, $c \rightarrow c + \langle EW_{Ca}\rangle \epsilon$, i.e. the
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output11/coeff0.pdf} 
            \caption{Model~I posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ in the $U$ band.
            ...
            enclosed probability.  Lines for zero value for $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ are shown for reference --
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output11/coeff1.pdf} 
            \caption{Posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ in the $B$ band, from Model~I.
 \label{global2:fig}}
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output11/coeff2.pdf} 
            \caption{Posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ in the $V$ band, from Model~I.
 \label{global3:fig}}
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.2in]{output11/coeff3.pdf} 
            \caption{Posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ in the $R$ band, from Model~I.
 \label{global4:fig}}
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
         \includegraphics[width=5.2in]{output11/coeff4.pdf} 
            \caption{Posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$, and $\sigma$ in the $I$ band, from Model~I.
 \label{global5:fig}}
\end{figure}
...
and $a(X)$, $b(X)$ of the \citetalias{1999PASP..111...63F} model (dashed lines).  All $\pmb{\gamma}$ vectors are chosen to intersect the origin.
...
the vectors $\pmb{\gamma}^0$ and $\pmb{\gamma}^1$.  Suppose that the data were described by a dust model.  Then a \citetalias{1999PASP..111...63F}  dust model 
can be written as 
...
The directions of the $\pmb{\gamma}$ vectors, and  the expectations from  \citetalias{1999PASP..111...63F} with $R=\kappa^{-1}$, are shown in 
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.2in]{output11/deltakk.pdf}
      \caption{
      \color{purple}
...
The imperfect correspondence between our model and that of   \citetalias{1999PASP..111...63F} is seen in Figure~\ref{sn2014j:fig}.
While both models reproduce the observed color excesses at the extreme $U$ and $I$ bands,  \citetalias{1999PASP..111...63F}'s
...
magnitudes similar
to two-parameter dust models that have been established for stars in the Milky Way, LMC, and SMC.
...
\label{dust2:eqn}
\end{equation}
In all aspects it is identical to the previous model except for the addition of the term $\pmb{\delta} D$,
which affects the mean
absolute magnitudes.  
Each supernova has an intrinsic $D$ parameter that is linearly related to
absolute magnitudes through global coefficients $\pmb{\delta}$.  As with $k_0$ and $k_1$,  $D$ is a latent supernova parameter whose existence is inferred from
its effects on broadband magnitudes.


The $\pmb{\delta} D$ terms contribute to the mean of the Normal distribution in Eq.~\ref{ewsiv2:eqn}, together with the spectral-feature terms:
for convenience these terms are referred to as ``intrinsic''.
However, switching the placement of the
$\pmb{\delta} D$ terms from Eq.~\ref{ewsiv2:eqn} to Eq.~\ref{dust2:eqn}, i.e.\ from intrinsic to extrinsic contributions,
does not significantly affect the results, meaning that data are not constraining enough
to identify $\pmb{\delta} D$ as being physically intrinsic or extrinsic.

\color{red}
We impose
$\langle D\rangle=0$
to specify the (arbitrary) definition of zero color.  We assign a prior on the ensemble amplitudes of $D$ to break the
degeneracy $\pmb{\delta} \rightarrow a\pmb{\delta}$, $D \rightarrow a^{-1}D$.  A sign degeneracy in $a$ remains.
In contrast to $\pmb{\gamma}$, our MCMC chains migrate between these degenerate solutions in  $\pmb{\delta}$.  This
degeneracy is preserved in the model.
In the running of the Monte Carlo fit, the initial conditions and ranges for
$\pmb{\gamma}^0$ and $\pmb{\gamma}^1$ were set to be in the neighborhood of the best fit of \S\ref{model:sec}
...
\color{red}
particularly in the $U$ and $B$ bands.
\color{black}
...
The significance of the $\pmb{\delta}$ parameters is shown pictorially in
Figure~\ref{deltacorner:fig}, which shows their posteriors.
Recall that fixing $\delta_X=0$ reverts this model into the two extrinsic-parameter model of \S\ref{model:sec},
this null result lies on the 85\%-ile contour.
Recall that
the model has a  $\pmb{\delta} \rightarrow -\pmb{\delta}$, $D \rightarrow -D$ that results in the bimodal peaks in the distribution.
To aid in the visualization of the results,
we break this degeneracy by imposing  that the one supernova with the highest signal-to-noise in $D$ be positive.
The resulting posterior contours of $\pmb{\delta}$ with this additional
condition are shown in Figure~\ref{deltacorner:fig}.
The modes of $\pmb{\delta}$ are at $\mp 1.12$, $\mp 0.89$, $\mp 0.36$,  $\mp0.16$, $\pm0.21$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Left:  Posterior contours for $\pmb{\delta}$ for Model~II.
   Right: The same posteriors after imposing that  the one supernova with the highest signal-to-noise has positive value of $D$.
     Lines for $\pmb{\delta}=0$ are overplotted.
   \label{deltacorner:fig}}
\end{figure}

An alternative visualization of the $\pmb{\delta}$ results can be seen in Figure~\ref{deltaratio:fig}, which
shows  values of $\delta_X/\delta_U-1$, statistics insensitive to the scaling (including the sign) degeneracy.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.2in]{output25/deltaratio.pdf}
   \caption{Median values of $\delta_X/\delta_U$ in the 5 bands and corresponding 68\% intervals for for Model~II.
...
%-----
mag respectively.
The range of $B-V$ colors is $> 5$ times larger for the external contribution; variation in $B-V$ color due to the intrinsic term is dwarfed by the extrinsic dust
component. 
...
\color{red}
The newly fitted $\pmb{\gamma}$ and  $\pmb{\delta}$ vectors (the latter normalized by $\delta_U$ for better signal-to-noise) can be expressed as
\color{black}
\begin{equation}
\begin{pmatrix}
 \frac{\gamma^0_X}{\gamma^0_B-\gamma^0_V} \\
...
(These are the same values found in the results of Model I in Eqs.\ \ref{trans_I:eqn}--\ref{res_I:eqn}.)
\color{red}
The $\vec{\epsilon}_{\delta}$ residual contributes 2\% of the length of the $\pmb{\delta}$ vector.
\color{black}


\color{red}
The analysis is evaluated using simulated datasets.
For one set of tests, we realize 20 random realizations of data with input  parameters equal to the median values found in
\S\ref{model:sec} and the same noise as our data.  The input values of $\pmb{\delta}$ are set to zero.
The posterior for each realization is analyzed individually, and the posteriors of all realizations are stacked.  The stack is sensitive to
biases in the best fit value, and is shown in Figure~\ref{simnulldelta:fig}.  The correlations between the parameters
and the bimodality of each posterior results in biases in the stack, with
modes at $\mp 0.67$, $\pm0.14$, $\pm0.13$,  $\mp0.03$, $\mp0.52$.
The input length of $\pmb{\delta}$ is zero, therefore any statistical
fluctuation biases the value high.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.2in]{output25_sim_null/delta_corner.pdf} 
            \caption{Stacked $\pmb{\delta}$ posterior contours for 20 independent simulated data sets with input $\pmb{\delta}=0$.
            \label{simnulldelta:fig}}
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
  ...
  \includegraphics[width=2.9in]{output25_sim_null/pvalues_relperp.pdf} 
            \caption{ $\pmb{\delta}^2$ $\delta_\perp^2/\delta^2$ for 20 independent simulated data sets with input $\delta=0$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output25_sim/coeff0.pdf} 
            \caption{Aggregated Model~II posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$,  $\pmb{\delta}$, and $\sigma$ in the $U$ band of ten independent simulated data sets.  Lines show the values of the input values.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output25_sim/coeff1.pdf} 
            \caption{Aggregated Model~II posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$,  $\pmb{\delta}$, and $\sigma$ in the $B$ band of ten independent simulated data sets.   Lines show the values of the input values.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output25_sim/coeff2.pdf} 
            \caption{Aggregated Model~II posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$,  $\pmb{\delta}$, and $\sigma$ in the $V$ band of ten independent simulated data sets.   Lines show the values of the input values.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.2in]{output25_sim/coeff3.pdf} 
            \caption{Aggregated Model~II posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$,  $\pmb{\delta}$, and $\sigma$ in the $R$ band of ten independent simulated data sets.   Lines show the values of the input values.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
         \includegraphics[width=5.2in]{output25_sim/coeff4.pdf} 
            \caption{Aggregated Model~II posterior contours for $c$, $\alpha$, $\beta$, $\eta$, $\pmb{\gamma}^0$, $\pmb{\gamma}^1$,  $\pmb{\delta}$, and $\sigma$ in the $I$ band of ten independent simulated data sets.   Lines show the values of the input values.
...
We stress that our $\pmb{\delta}$ vector does not capture the full behavior of the observable effects of the $D$ parameter, but rather only
the contribution perpendicular to the $\pmb{\gamma}$ vectors.
