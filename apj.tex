...
%------
confidence.
The color changes allowed by the model are almost parallel to the color changes allowed by the dust model of  \citet{1999PASP..111...63F} with floating
total-to-selective extinction parameter $R^F$.  The prior of no parameter covariance used in the training, and the deduced extinctions and color excess
are both qualitatively consistent with $R^F \sim 2.4$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=5.2in]{output11/residual.pdf} 
            \caption{Differences between predicted versus observed colors versus the observed color.  For each color, the ideogram of the color differences is
            shown on the right.
            \label{residual:fig}}
\end{figure}
...
\color{red}
[Greg asked for discussion but I think the spectral correlations should go in Chotard et al.\ 2017.]
...
\color{orange}
For brevity, we do not present here the analysis of simulated data used to validate and bias quantify bias in our results.
...
2-dimensional color variations that are closely aligned with the 2-dimensional color variations predicted by  \citetalias{1999PASP..111...63F} dust.
...
When fixing the origin of the $\gamma$'s to zero, the are found to be coplanar with $a$ and $b$.
...
As a check of the above interpretation, we generate a simulated dataset with the same numbers and uncertainties as our data, based on a
SALT2 $x_1=0$, $c=0$ template and  \citetalias{1999PASP..111...63F} dust.  The supernovae
are assigned $E^F(B-V)$ parameters equal to the $B-V$ color excesses determined in our sample, and $R^F$ values drawn from a log-normal
distribution with a mean of $\ln{2.4}$ and 0.05 standard deviation.  The $\alpha$, $\beta$, and $\eta$ parameters are set to zero, so that dust and
measurement uncertainty are the only sources of variation in magnitudes.  
From the Monte Carlo chains, we find a 68\% credible interval for $\kappa^{-1}_1$ of 2.18--2.40.
As with the analysis of our data, we find that the
$\gamma$'s and \citetalias{1999PASP..111...63F}  vectors for nearly parallel planes, with the $\gamma^0$ vector almost consistent with
$\kappa_1^{-1}=2.4$, as expected given the mean of $\ln{2.4}$ of the input log-normal distribution.
...
median values of Left: $\gamma^0_V k_0$, which is associated up to an offset with the $A^F_V$ of an $R^F \sim 1/\kappa_1$ dust model; Right: $\gamma^1_V k_1$,
...
\begin{equation}
E_o(X-V) =  \left(\frac{\gamma^0_X}{\gamma^0_V}-1\right)e_0 +  \left(\frac{\gamma^1_X}{\gamma^1_V}-1\right)e_1,
\end{equation}
...
To constrain the new degrees of freedom and degeneracies inherent in the model we impose additional constraints.  We first
require that
...
\end{equation}
The first of these constraints specifies the definition of zero color.   The latter two confine possible magnitude
...
regression dilution.  Our analysis is susceptible to this bias because we do consciously do not include distributions
