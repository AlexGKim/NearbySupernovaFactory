...
three \added{latent} color parameters and dependence on the equivalent widths of Ca~II and Si~II, the Si~II velocity,
...
\added{By latent, we mean that the parameters are not directly associated with a single observable but rather deduced
...
Unlike
the spectral parameters ($EW_{Ca}$, $EW_{Si}$ and $\lambda_{Si}$),  $g_0$ and $g_1$ are not directly associated
...
\added{The notation used in this article is not to be associated with the those from previous works.}
...
\added{Results in this article are given in terms of $\vec{\gamma}g$, not $g$ alone.
}
...
\replaced{
We  thus interpret}{In this section we reexpress our model $\vec{\gamma}$'s in terms of}
...
\added{The values of $\kappa$'s are determined by the conditions that the components $A^F_V - \kappa_2 E^F(B-V)$ and $-\kappa_1 A^F_V + E^F(B-V)$ be
...
\added{parameter uncertainty between supernovae; recall the $\langle \Delta \rangle=0$, $\langle g_0 \rangle=0$, $\langle g_1 \rangle=0$,
...
\added{The significantly non-zero values for  $\alpha_{\hat{U}}$ and $\beta_{\hat{U}}$ from this study can be compared with the $U$-band study of
...
\added{There is no sign of two populations in $\gamma^1_{\hat{V}} g_1$, which would be expected from a subpopulation
experiencing circumsteallar dust.}.
...
\end{table}
\section{Degeneracy}
Global parameters $\alpha$, $\beta$.  Per supernova parameters $\mu_i$, $x_i$, $c_i$, $M_i$.

There are two arbitrary numbers $\epsilon_\alpha$ and $\epsilon_\beta$.

The per supernova expected magnitude is

\begin{align}
m_i & = \mu_i + \langle M\rangle + \delta_{mi}  -\alpha x_i + \beta c_i\\
& = \mu_i + \langle M\rangle + \epsilon_\alpha x_i - \epsilon_\beta   c_i+ \delta_{mi}  -(\alpha+ \epsilon_\alpha) x_i + (\beta+\epsilon_\beta) c_i\\
& = \mu_i + \langle M\rangle' + \delta'_{mi}  -\alpha' x_i+ \beta' c_i .
\end{align}
where
$\langle M\rangle' = \langle M\rangle + \epsilon_\alpha \langle x_i\rangle  - \epsilon_\beta   \langle c_i \rangle$ and
$\delta'_{mi} =  \delta_{mi} +\epsilon_\alpha (x_i-\langle x_i \rangle) - \epsilon_\beta   (c_i-\langle c_i\rangle)   $, $\alpha' =\alpha+\epsilon_\alpha$, $\beta' =\beta+\epsilon_\beta$.
There are two degrees of freedom between the unprimed
and primed variables of Eqs.~1 and 2.

Impose $\delta_{mi} \sim \mathcal{N}(0,\sigma_\delta)$.  If $x_i$ and $c_i$ have Normal distributions this condition does not break the degeneracy.
A Baysian model (e.g.\ UNITY) that assumes Normal distributions for these parameters thus does not break the degeneracy.
If  $x_i$ and $c_i$ do not have Normal distributions, the prior will drive $\epsilon$ to 0.

SUGAR assumes normal distributions for its features.  It breaks its  degeneracy  by enforcing the analogue of $cov(\delta_{mi}, x_i)=0$ and $cov(\delta_{mi}, c_i)=0$.

I do not impose the distribution from which $\Delta$ comes.
