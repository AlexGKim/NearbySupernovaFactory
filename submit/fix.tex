...
This section describes the evaluated posterior of the model parameters.  Before doing so, we show that the model provides
a fair representation of the data.
As a diagnostic that the model results do not give catastrophically poor color predictions and are not pulled by extreme
outlying data, shown in Figure~\ref{residual:fig} are plots of observed  versus
...
and the histogram of their differences.  The residuals of the data away from the model have a compact Gaussian-like appearance, and
show no bias with respect to observed color.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.2in]{../output_fix3_x1/residual.pdf} 
...
Each chain converges to one of the  $\vec{\phi}$--$p$ degenerate solutions, away from which it does not migrate.  The 
confined peak of the posterior surface, in addition to the MCMC-convergence tests described in  \S\ref{results1:sec}, provide evidence that the MCMC has not failed to converge.
...
We find significant non-zero values for $\vec{\alpha}$, $\vec{\beta}$, and $\vec{\eta}$ indicating that
$EW_{Ca}$, $EW_{Si}$, and $\lambda_{Si}$ are indicators of broadband
colors at peak
as expected.  The non-zero values for $\vec{\zeta}$ demonstrate that light-curve shape $x_1$ is also an indicator
of broadband colors.
...
\citep{2008A&A...492..535A, 2011A&A...529L...4C}
and exists
in our sample as is shown late in  this section.  It is not surprising that our analysis does not entirely distinguish between
their influence on supernova colors. Nevertheless, their posteriors are bounded and inconsistent with zero, so they do convey independent information.
It is for this reason  that Model~III is highlighted in this article over Model~II.

The posterior gives a significant magnitude influence of the $p$ parameter.
...
The median data color uncertainty of 0.010~mag (the maximum is 0.042~mag) so we conclude that
the is not a due to spurious noise.

The chromatic effect of $\vec{\phi}$ is visualized in Figure~\ref{phiratio:fig}, which
gives a different perspective of the posterior using the statistic $\phi_X/\phi_{\hat{I}}-1$.
Unlike with $\gamma$, we normalize $\phi$ to $\hat{U}$ because the uncertainty in $\phi_{\hat{B}}-\phi_{\hat{V}}$ is relatively large with
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.2in]{../output_fix3_x1/phiratio.pdf}
   \caption{Model~III median values  and corresponding 68\% intervals for $\phi_X/\phi_{\hat{I}}$ in the 5 bands.
...
The direction of the vector is shown in Figure~\ref{plane3:fig}.
...
onto the dust plane is
$0.959^{+0.015}_{-0.029}$.
The corresponding 68\% credible is $R^F_{\mathit{eff}}=2.40^{+0.15}_{-0.15}$.
...
An intuitive way to consider the effect of $\vec{\gamma}^1$ is through the deviation from linearity in the color-magnitude diagram.
A matrix such as the one in  Eq.~\ref{trans_I:eqn} is used to transform this $\{g_0, g_1\}$ into an $\{A_V^F, E^F(B-V)\}$ pair
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.2in]{../output_fix3_x1/avebv_synth.pdf}
      \caption{
...
$E_{\gamma^0}({\hat{B}}-{\hat{V}})=(\gamma^0_{\hat{B}}-\gamma^0_{\hat{V}})k_0$, $E_{\gamma^1}({\hat{B}}-{\hat{V}})=(\gamma^1_{\hat{B}}-\gamma^1_{\hat{V}})k_1$, and $p\sigma_p\phi_{\hat{I}}$.
...
\end{pmatrix}.
\end{multline}
To aid visualization, only the upper triangular part of the otherwise symmetric matrix is shown and the corresponding feature label is given on
the diagonal.

We find $>2 \sigma$ correlations between our extrinsic color-excess parameters and the input features  $EW_{Ca}$,
$EW_{Si}$, and $x_1$.
...
As alluded to earlier, the feature $EW_{Si}$ is strongly correlated with $x_1$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.2in]{../output_fix3_x1/deltaDelta_hist.pdf} 
   \caption{
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=3.5in]{../output_fix3_x1/childress.pdf} 
...
and the  stack of the posteriors of all supernovae  for $\gamma^0_{\hat{V}} k_0$ 
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{delta_corner.pdf}
     \includegraphics[width=2.8in]{delta_corner_flipped.pdf}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
      \includegraphics[width=5.2in]{deltaratio.pdf}
...
The distributions of both sources of color excess $E({\hat{B}}-{\hat{V}})$ and
 $E_\delta({\hat{B}}-{\hat{V}}) \equiv   (\delta_{\hat{B}}-\delta_{\hat{V}})D$ are shown in Figure~\ref{ebv:fig} as the  stack of the posteriors of all supernovae 
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{ebv.pdf}
   \includegraphics[width=2.8in]{ebv_delta.pdf}
...
three different $p$-values ranging from 
\color{blue}
0.87--0.90,
\color{black}
 indicating that the
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.9in]{pvalues_delta2.pdf} 
  \includegraphics[width=2.9in]{pvalues_relperp.pdf} 
