...
Correspondingly the sample has an effective value of $\langle R^F_{\mathit{eff}}\rangle \sim 2.40$, though with a broad distribution of 
...
\begin{pmatrix}
\begin{array}{rr}
39.7^{+2.4}_{-2.5} & 16.6^{+0.6}_{-0.6}  \\
-14.7^{+3.4}_{-3.4} & 3.7^{+1.0}_{-0.9}  
\end{array}
\end{pmatrix} 
\end{equation}
...
\begin{align}
\begin{split}
\vec{\epsilon}_{\gamma^0} &=\left(1.0^{+0.4}_{-0.4} , -1.2^{+0.4}_{-0.4} , -0.8^{+0.4}_{-0.4} , 0.7^{+0.2}_{-0.2} , 0.4^{+0.6}_{-0.6}\right), \\
\vec{\epsilon}_{\gamma^1} & =\left(-2.7^{+0.6}_{-0.6} , 4.3^{+0.6}_{-0.6} , -0.8^{+0.6}_{-0.6} , 1.2^{+0.3}_{-0.3} , -2.6^{+0.9}_{-0.9}\right).
\end{split}
\label{res_3:eqn}
\end{align}
...
onto this plane is
$0.959^{+0.015}_{-0.029}$.
The   $\vec{\gamma}^0$  vector points in the $\langle R^F_{\mathit{eff}}\rangle=2.40^{+0.15}_{-0.15}$ dust-extinction direction.
...
\label{g0g1:sec}
The vector
$\vec{\gamma}^0$ describes color changes for an effective $\langle R^F_{\mathit{eff}}\rangle=2.40$ dust, as shown 
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{../output_fix3_x1/deltagamma1_med.pdf}
      ...
      Normalized stack of the posteriors of all supernovae  and histogram 
      of  per-supernova 
median values of: (left) $\gamma^0_{\hat{V}} g_0-\gamma^0_{\hat{V}} g_0|_0$, which is associated with the $A^F_V$ of an $\langle R^F_{\mathit{eff}}\rangle \sim 2.4$ dust model;
...
For reference, a line that represents $\langle R^F_{\mathit{eff}}\rangle=2.4$ is overplotted; points above
...
to be $\phi_{\hat{B}}/(\phi_{\hat{B}}-\phi_{\hat{V}}) = -1.9^{+1.0}_{-2.4}$, which appears to be inconsistent with  \citetalias{2017ApJ...842...93M}.  
...
the Kolmogorov-Smirnov test gives a two-tailed $p$-value of $ 0.02 _ {   -0.02 } ^{   + 0.04 }$.
...
for low- and high-mass hosts respectively, with a difference of $0.013 \pm {     0.005 }$~mag that is significant at $>2 \sigma$.
...
with an effective
$\langle R^F_{\mathit{eff}}\rangle \sim 2.40$ and an asymmetric tail extending toward lower values.
We identify a new
...
that we and  \citetalias{2017ApJ...842...93M}  are seeing the same intrinsic color and to confirm the
