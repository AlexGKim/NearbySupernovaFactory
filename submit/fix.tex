...
assumed to be drawn from a Normal distribution with unit standard deviation; leaving the distribution unconstrained,
as was done for $g_0$ and $g_1$, yields many local maxima in the posterior and makes MCMC convergence difficult.
...
Model~II introduces a new parameter  that can absorb color varability
that was restricted to $g_0$ and $g_1$ in Model~I, and whose distribution comes from a Normal distribution.
The change in the distributions of the $\gamma^0_{\hat{V}} g_0$ and $\gamma^1_{\hat{V}}g_1$ parameters from
 Model~I to Model~II
is apparent in Figure~\ref{kcomp:fig}.  (Here and in future plots, the parameter value of one supernova,
...
where it seems that a Normal distribution component in Model~I disappears in Model~II, presumably having been  assigned to $p$.
...
Model~III explicitly includes light-curve shape as a feature used to standardize supernova colors, in addition to the
...
$x_1$, through linear coefficients $\zeta$. 
The model reads as
\begin{equation}
p  \sim \mathcal{N}(0,1 ),
\end{equation}
...
The algorithm is run with input simulated data with known  signals and measurement uncertainties similar to those of our dataset.
...
As will be seen in  \S\ref{results3global:sec}, the final posterior has a distinct bounded  
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=4in]{../output_fix3_x1/residual.pdf} 
            ...
            Model~III-expectation colors (including  contributions related to spectral features, light-curve shape, $\gamma g$, and $\phi p$  terms)
...
we set $\vec{\phi} = \text{sign}(\phi_{\hat{I}}) \vec{\phi}$ to break its parity degeneracy;
Figure~\ref{global5:fig} shows there the degenerate solutions are relatively well separated 
in $\sigma_p \phi_{\hat{I}}$.
...
The story on the Si~II~$\lambda$6355 is not as clear.
...
This is as expected based on the results of \citet{2007A&A...466...11G} and MLCS2k2
\citep{2007ApJ...659..122J}.
}
...
\subsubsection{Results for the Extrinsic Latent Parameter Cofactors $\vec{\gamma}^0$, $\vec{\gamma}^1$}
...
The posterior gives a quadratic fraction of $0.9996^{+0.0002}_{-0.0003}$ of $\vec{\gamma}^0$ that projects onto the plane of
...
onto this plane is
$0.959^{+0.015}_{-0.029}$.
The   $\vec{\gamma}$  vectors project  onto an $R^F_{\mathit{eff}}=2.40^{+0.15}_{-0.15}$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{
   Visualization of how supernova magnitudes can vary in Model~III and that of \citetalias{1999PASP..111...63F}.
...
with $\sigma_p = 0.049^{+ 0.004}_{-0.003}$~mag.  The smallest value of $\sigma_p$
over all Monte Carlo chains is 0.039~mag.
...
a reddening $\hat{B}-\hat{V}$ is accompanied with a bluing in $\hat{V}-\hat{R}$.  The reddening and bluing are accentuated 
in the extreme $\hat{U}$ and $\hat{I}$ bands. Absorption and re-emission of light
over broad bands are the most obvious candidate for producing this behavior, a process
...
To temper our physical interpretation of $p$, 
we emphasize that
...
There is likely more than one physical affect being attributed to $p$, leading to a $\sigma_p \vec{\phi}$ that does not correspond to a single process.
...
$E_{\gamma^0}({\hat{B}}-{\hat{V}})=(\gamma^0_{\hat{B}}-\gamma^0_{\hat{V}})g_0$, $E_{\gamma^1}({\hat{B}}-{\hat{V}})=(\gamma^1_{\hat{B}}-\gamma^1_{\hat{V}})g_1$, and $A_{p,I} =  \sigma_p\phi_{\hat{I}}p$.  (The  three latent parameters are multiplied by select cofactors
that give the physically color excess and extinction.)
The distributions of the Monte Carlo links for all supernovae are shown in Figure~\ref{perobject3:fig}.
Each supernova is represented by a cloud of its parameters from all links.
...
\begin{multline}
Cor(\Delta, EW_{Ca}, EW_{Si}, \lambda_{Si}, x_1, E_{\gamma^0}({\hat{B}}-{\hat{V}}), E_{\gamma^1}({\hat{B}}-{\hat{V}}), \sigma_p\phi_{\hat{I}}p ) =\\
\begin{pmatrix}
...
\ldots  & \ldots & \ldots & \ldots  & \ldots  & \ldots  &\ldots  & A_{p,I} \\
\end{array}
\end{pmatrix}.
\label{corr:eqn}
\end{multline}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{
   Distributions for the supernova parameters $\Delta$, $EW_{Ca}$, $EW_{Si}$, $\lambda_{Si}$, $x_1$, $E_{\gamma^0}({\hat{B}}-{\hat{V}})$,  $E_{\gamma^1}({\hat{B}}-{\hat{V}})$,  and  $A_{p,I}$, as well as the grey offset
...
\label{g0g1:sec}
The vector
$\vec{\gamma}^0$ describes color changes for an effective $R^F_{\mathit{eff}}=2.40$ dust, as shown 
In  \S\ref{results3gamma:sec}.
Hence, the per-supernova parameter $\gamma^0_{\hat{V}} g_0$ is associated with the $\hat{V}$-band
extinction for a constant $R^F=2.40$.
}
...
(Subtracting the value of the one supernova suppresses correlated errors common to all supernovae.)
...
which  is associated with extinction corrections due to deviations away from the canonical $R^F_{\mathit{eff}}$ value.
   \label{k0_med:fig}}
\end{figure}
...
For reference, a line that represents $R^F_{\mathit{eff}}=2.4$ is overplotted; points above
...
the magnitudes or colors of a supernova that does not experience dust.  Without further assumptions, it cannot give
per-supernova determinations of $R^F$.
...
The correlation between $EW_{Si}$ and host has  already been noted \citep{2011ApJ...734...42N}.
There is a lack of correlation between  $g_0$, $g_1$ and  our velocity parameter $\lambda_{Si}$.
This contrasts with studies of high-velocity features (HVF) identified using
Si~II~$\lambda$6355 that find correlations with host galaxy properties
\citep[e.g.][]{2014MNRAS.444.3258M, 2015MNRAS.446..354P, 2015ApJS..220...20Z}.
The population and properties of HVF SNe within the SNfactory sample will be presented in \citep{2018lin}.
The correlation of $x_1$ with with host-galaxy (including dust) properties is  well established 
