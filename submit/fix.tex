...
an empirical model for SN~Ia peak magnitudes with two color parameters and dependence on the equivalent widths of Ca~II and Si~II, and Si~II velocity
...
total-to-selective extinction parameter $R^F$. 
...
%-----
$\langle R^F_{\mathit{\mathit{eff}}}\rangle$ is 2.31 -- 2.69.
%-----
...
The 
spectroscopic-feature measurements, SALT light-curve fit parameters including the shape  $x_1$,
and uncertainties
are presented
...
\section{Model~I: Extrinsic Colors}
\label{modelI:sec}
We begin by considering a model for supernova magnitudes that have linear dependence on spectral parameters
and two extrinsic color parameters. This model is referred to as Model~I.  It is not the primary focus of this article, but its
results are of interest.  The purpose of this Section is to introduce some basic concepts of our models,
to show how the models can be expressed in terms of dust-extinction models, and to give Model~I results on dust. 
We refrain from presenting the full results of Model~I in deference to the more complex Model~II that is the subject of Section~\ref{modelII:sec},
...
the combination of all components are written as a vector, e.g.\ $\vec{c}=(c_{\hat{U}}, c_{\hat{B}}, c_{\hat{V}}, c_{\hat{R}}, c_{\hat{I}}) )$,.  This 
notationally distinguishes between global versus per-supernova parameters.
...
Except where stated otherwise above, we adopt a flat prior for all parameters.
...
(recall the constraint in Eq.~\ref{zero:eqn}),  and $5 \times 7$ global coefficients.
For $N=172$ supernovae, there are 1376 observables and 1064  parameters.

\subsection{Results: $\gamma^0$ and $\gamma^1$}
\label{results1:sec}
In this subsection we concentrate on the results for $\vec{\gamma}^0$ and $\vec{\gamma}^1$
(the 68\% credible intervals for all global parameters are given in \S\ref{m12results:sec}.)
...
Here we  interpret our model $\gamma$'s in terms of
the dust-extinction model of \citetalias{1999PASP..111...63F}.
...
our Monte Carlo chains for $\vec{\gamma}$ come close to the origin, we thus claim detection of the
influence of $g_0$ and $g_1$  on supernova magnitudes
with probability $1-5\times 10^{-5}$.
We thus find a strong signal for a linear 
extrinsic contribution to our model
\begin{equation}
A_X = \gamma^0_X g_0 +  \gamma^1_X g_1.
\end{equation}

A linear model with parameters $A_V$ and $E(B-V)$
\begin{equation}
A_X = a_X  A_{V} + b_X E(B-V)
\label{f99:eqn}
\end{equation}
approximately describes  
dust-extinction
models.  For example,   the wavelength-dependent model of  \citet{1989ApJ...345..245C} is linear,
...
Our model $\gamma$ vectors can be written as a linear combination (described by the $2 \times 2$ matrix M) of the
dust-based $a_X$ and $b_X$ vectors plus residual vectors perpendicular to $\vec{a}$ and $\vec{b}$,
\begin{equation}
\begin{pmatrix}
\vec{\gamma}^0 \\
\vec{\gamma}^1
...
\label{trans_I:eqn}
\end{equation}
The $\gamma$'s are almost entirely captured by the first term. 
The Model~I posterior gives a quadratic fraction of $0.9993^{+0.0002}_{-0.0002}$ of $\vec{\gamma}^0$ projected onto the plane defined by
$a_X$ and $b_X$, the quadratic fraction of $\vec{\gamma}^1$ projected
onto the dust plane is
$0.959^{+0.013}_{-0.024}$.
...
Comparing this equation with  Eq.~\ref{trans_I:eqn}, we see that $\kappa_1=M[1,2]/M[1,1]$, $\kappa_2=M[2,1]/M[2,2]$.
...
All color changes would then be confined to the $a_X+\kappa_1 b_X$ direction. This scenario corresponds to the \citetalias{1999PASP..111...63F}  dust model restricted to a constant $R^F=\kappa_1^{-1}$.   We use this interpretation to assign  $\langle R^F_{\mathit{eff}}\rangle = \kappa_1^{-1}$ as
the
ensemble mean
effective total-to-selective extinction of our sample.
With our posterior we
define an effective $R^F_{\mathit{eff}} \equiv \kappa^{-1}_1=2.18^{+0.14}_{-0.16}$.
Figure~\ref{plane:fig} visualizes the near-perfect alignment of $\vec{\gamma}^0$ and  $\vec{a}+\vec{b}/2.18$.

In the above context, the direction of the $\vec{\gamma}^1$ vector is described by $\kappa_2$.
The prior that $g_0$ and  $g_1$ are uncorrelated leads  to  $\kappa_2$ being determined
by the sample 
distribution of $A^F_V$ and $E^F(B-V)$.  In this article, we do not further consider the direction of
$\vec{\gamma}^1$ but rather focus on the distribution of the per-supernova parameters.
...
\subsection{Remarks}
Model~I is not of primary interest in this article.  However, there are  points to highlight from its results.

\begin{itemize}
\item Our model introduces two latent supernova parameters that can influence supernova colors with ``non-informative'' priors.  Fitting
...
This lends confidence to our modelling and analysis.
...
those from two-parameter dust extinction contribute to those two parameters.   The effective $R^F$ we derive from our results
is susceptible to leakage from intrinsic colors not associated with dust.
\end{itemize}

\section{Model~II: Extrinsic--Intrinsic Colors}
\label{modelII:sec}
...
the optical depth of dust, one parameter to capture variation in dust properties, and
intrinsic color variability unrelated to dust.  In this section we  focus on describing the consequences of adding the new parameter.

\subsection{Model}
\label{modelIImodel:sec}
Model~II is the same as Model~I except for the 
addition of an intrinsic color parameter $p$.  The distribution of this parameter over all supernovae is
assumed to be drawn from a Normal distribtion
\begin{equation}
p \sim \mathcal{N}(0, \sigma_p).
\end{equation}
The influence of $p$ on supernova colors is described by the global parameters $\vec{\phi}$ (constrained to be a
unit vector)
so that the color offset of each supernova is  $\vec{\phi}p$.
Model~II reads as
\begin{equation}
\begin{pmatrix}
{\hat{U}}\\{\hat{B}}\\{\hat{V}}\\{\hat{R}}\\{\hat{I}}
...
\Delta +
\begin{pmatrix}
c_{\hat{U}}+\alpha_{\hat{U}} EW_{Ca} + \beta_{\hat{U}} EW_{Si} + \eta_{\hat{U}} \lambda_{Si} + \phi_{\hat{U}} p\\
c_{\hat{B}}+\alpha_{\hat{B}} EW_{Ca} + \beta_{\hat{B}} EW_{Si} + \eta_{\hat{B}} \lambda_{Si}  + \phi_{\hat{B}} p\\
c_{\hat{V}}+\alpha_{\hat{V}} EW_{Ca} + \beta_{\hat{V}} EW_{Si} + \eta_{\hat{V}} \lambda_{Si} + \phi_{\hat{V}}  p\\
c_{\hat{R}}+\alpha_{\hat{R}} EW_{Ca} + \beta_{\hat{R}} EW_{Si} + \eta_{\hat{R}} \lambda_{Si} + \phi_{\hat{R}} p\\
c_{\hat{I}}+\alpha_{\hat{I}} EW_{Ca} + \beta_{\hat{I}} EW_{Si}+ \eta_{\hat{I}} \lambda_{Si}+ \phi_{\hat{I}}  p
\end{pmatrix},
\label{ewsiv2:eqn}
\end{equation}
\begin{equation}
...
use ``intrinsic'' and ``extrinsic'' as descriptive labels for $\phi$ and $\gamma$ respectively, the model cannot identify
their physical origins.

The model contains the product of latent parameters
$\vec{\phi} p$, which leads to the degeneracy $\vec{\phi} \rightarrow -\vec{\phi}$, $p \rightarrow -p$.
No conditions are added to break this degeneracy as we want to explore the possibility that
$\sigma_p\phi$ is consistent with zero.  The degeneracy
in $\vec{\phi}$ is apparent later in the article, but it does not affect the relevant science that
comes with the combination $\vec{\phi} p$.

For $N$ supernovae there are $8N$ observables.  There are $3N$ spectral parameters, each of
$\Delta$, $g_0$, $g_1$ contributes $N-1$ parameters, there are $N$ parameters for $p$ plus one for their standard
deviation $\sigma_p$,  and $5 \times 7$ global coefficients.
For $N=172$ supernovae, there are 1376 observables and 1237  parameters.

\subsection{Results: Effect of the Third Color Term}
\label{results2:sec}
In this subsection we focus on the effect of adding the third color term  $\sigma_p \vec{\phi}$.
The 68\% credible intervals for relevant global parameters are given in \S\ref{m12results:sec}.
...
The two extrema are due to the sign degeneracy for $\vec{\phi}$, each of the MCMC chains converges to one of the solutions and does
not migrate between them.
The direction of $\vec{\phi}$ is not aligned with the plane defined by
the $\vec{\gamma}$ vectors; the 68\% credible interval for the  quadratic fraction of
$\vec{\phi}$ in the $\vec{\gamma}_0$--$\vec{\gamma}_1$ plane is $0.088^{+0.118}_{-0.063}$.
...
Model~II introduces a degree of freedom that is associated with a Normal distribution.
It is thus interesting to compare the distributions of the $g_0$ and $g_1$ parameters of Model~I and Model~II,
which are shown in Figure~\ref{kcomp:fig}.  The apparent difference between Model~I and II is in the $\gamma^1g_1$ term,
where it seems that a Normal distribution component in Model~I is no longer in Model~II but is  assigned to the $\sigma_P \vec{\phi}$.
...
$a_X$ and $b_X$ of the \citetalias{1999PASP..111...63F} model.
The 68\% credible interval is $R^F_{\mathit{eff}} = 2.54^{+0.20}_{-0.20}$.

\subsection{Remarks}
The inclusion of a third color term beyond the two (which are expected from dust-extinction models) of Model~I has important consequences.
...
\item The data strongly supports the influence of an additional supernova parameter that affects supernova colors in a manner orthogonal to
 dust extinction.
\item The effective $R^F_{\mathit{eff}}$ shifts by $>1 \sigma$ when including the new color term.  This demonstrates,
as has been noted by \citet{2011A&A...529L...4C}, that dust analyses
that do not allow for additional color dispersion are susceptible to bias in inferred dust parameters.
\end{itemize}



\section{Model~III: Extrinsic--Intrinsic Colors and Light-Curve Shape}
\label{modelIII:sec}
The two models already presented standardize supernovae based on spectral features and colors at peak brightness.
Nevertheless we know that multi-epoch data also trace supernova properties.
Light-curve shape is an established indicator of SN~Ia diversity 
\citep{1993ApJ...413L.105P, 1996ApJ...473...88R,
1997ApJ...483..565P}
associated with color \citep{1996ApJ...473...88R,2005A&A...443..781G}.
Model~III introduced in this section explicitly includes light-curve shape
along with the spectral features.  This model is of primary interest
in this article and its results are described in depth.

\subsection{Model}
\label{modelIImodel:sec}
Model~III is the same as Model~II except for the 
addition of a new term  that relates magnitudes with the SALT2 light-curve-shape parameter,
$x_1$, that is related to magnitudes with linear coefficients $\zeta$. 
The model reads as
\begin{equation}
p \sim \mathcal{N}(0, \sigma_p),
\end{equation}
...
\Delta +
\begin{pmatrix}
c_{\hat{U}}+\alpha_{\hat{U}} EW_{Ca} + \beta_{\hat{U}} EW_{Si} + \eta_{\hat{U}} \lambda_{Si} +\zeta_{\hat{U}} x_1 + \phi_{\hat{U}} p\\
c_{\hat{B}}+\alpha_{\hat{B}} EW_{Ca} + \beta_{\hat{B}} EW_{Si} + \eta_{\hat{B}} \lambda_{Si} +\zeta_{\hat{B}} x_1 + \phi_{\hat{B}} p\\
c_{\hat{V}}+\alpha_{\hat{V}} EW_{Ca} + \beta_{\hat{V}} EW_{Si} + \eta_{\hat{V}} \lambda_{Si} +\zeta_{\hat{V}} x_1+ \phi_{\hat{V}}  p\\
c_{\hat{R}}+\alpha_{\hat{R}} EW_{Ca} + \beta_{\hat{R}} EW_{Si} + \eta_{\hat{R}} \lambda_{Si} +\zeta_{\hat{R}} x_1+ \phi_{\hat{R}} p\\
c_{\hat{I}}+\alpha_{\hat{I}} EW_{Ca} + \beta_{\hat{I}} EW_{Si}+ \eta_{\hat{I}} \lambda_{Si}+\zeta_{\hat{I}} x_1+ \phi_{\hat{I}}  p
\end{pmatrix},
\label{ewsiv3:eqn}
\end{equation}
\begin{equation}
...
\label{dust3:eqn}
\end{equation}
The only change from Model~II is the addition of the $\vec{\zeta} x_1$ terms in Eq.~\ref{ewsiv3:eqn}.

For $N$ supernovae there are $9N$ observables.  There are $3N$ spectral parameters, each of
$\Delta$, $g_0$, $g_1$ contributes $N-1$ parameters, there are $N$ parameters for $p$ plus one for their standard
deviation $\sigma_p$,  and $6 \times 7$ global coefficients.
For $N=172$ supernovae, there are 1548 observables and 1409  parameters.

\subsection{Results}
\subsubsection{Analysis Method and Validation}

In the analysis of this and other sections of this article,
...
The algorithm is extensively tested with simulated data with known input signals and measurement uncertainties similar to those of our dataset.
...
The posterior that will be shown in  \S\ref{results3global:sec} exhibits a 
confined peak, exhibiting no evidence that the MCMC has not failed to converge.

The model provides a fair representation of the data, as seen in the difference between the data and the model prediction.
Figure~\ref{residual:fig} has plots of the observed  versus
model-expectation colors (including  supernova-specific spectral features, light-curve shape $\gamma g$, and $\phi p$ terms) for four selected colors,
and the histogram of their differences.  
There are no extreme residuals 
from catastrophically poor color predictions.  There are no apparent trends between residuals and observed
color.   The residuals of the data away from the model have a compact Gaussian-like appearance.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4in]{../output_fix3_x1/residual.pdf} 
            \caption{Observed  versus
            Model~III-expectation colors (including  supernova-specific spectral features, $\gamma g$, and $\phi p$  terms)
...
Results from our analysis are shown through contours of the posterior surface for pairs of global parameters grouped by filter, 
in Figures~\ref{global1:fig} -- \ref{global5:fig}.
The confidence regions are localized and unimodal, except for the combination $\sigma_p \vec{\phi}$ that
has the sign degeneracy already noted in \S\ref{results2:sec} and plotted in Figure~\ref{M1ev:fig}.
Within the finite number of generated links, the MCMC chains do not migrate to the parity-degenerate solution of $\vec{\gamma}$ and $g$.
Each chain converges to one of the  $\vec{\phi}$--$p$ degenerate solutions, away from which it does not migrate.  
...
To break the sign degeneracy, we enforce that $\phi_{\hat{I}}$
be positive such that  $\vec{\phi} = \text{sign}(\phi_{\hat{I}}) \vec{\phi}$;
we use the $\hat{I}$-band because
Figure~\ref{global5:fig} shows that it has
the largest separation between the degenerate solutions of $\sigma_p \phi_X$.
...
The freedom to scale each of the terms of $\vec{\gamma}g$ is nulled out in the statistic
 $\gamma^i_X/\gamma^i_{\hat{V}}-1$.
Note the analogy between these statistics and the common use of
...
& $0.0033^{+0.0006}_{-0.0007}$
& $0.0029^{+0.0006}_{-0.0006}$
& $0.0026^{+0.0005}_{-0.0005}$
& $0.0041^{+0.0004}_{-0.0005}$
\\
${\alpha_X/\alpha_V-1}$
& $   1.0^{+   0.2}_{  -0.2}$
& $   0.1^{+   0.1}_{  -0.1}$
...
\\
$\eta_X$
& $0.0022^{+0.0011}_{-0.0011}$
& $0.0024^{+0.0009}_{-0.0010}$
& $0.0027^{+0.0008}_{-0.0008}$
...
\hline
\end{tabular}
\caption{68\% credible intervals for the global fit parameters of the Extrinsic--Intrinsic Model~III in \S\ref{modelIII:sec}.\label{global3:tab}}
\end{table}

\subsubsection{$\vec{\alpha}$, $\vec{\beta}$, $\vec{\eta}$,  $\vec{\zeta}$: Cofactors of Observables}
...
colors at peak.  The non-zero values for $\vec{\zeta}$ demonstrate that light-curve shape $x_1$ is also an indicator
of broadband colors.
...
Nevertheless the $\vec{\zeta}$ and $\vec{\beta}$   posteriors are bounded and inconsistent with zero, indicating that
 $x_1$ and $EW_{Si}$ do convey independent color information.
For this reason  Model~III is highlighted in this article over Model~II.

\subsubsection{$\vec{\gamma}^0$, $\vec{\gamma}^1$: Cofactors of Extrinsic Latent Parameters}

As with Model~I in \S\ref{modelI:sec}, all elements of the two $\vec{\gamma}$ parameter vectors 
describing extrinsic color
...
The posterior gives a quadratic fraction of $0.9996^{+0.0002}_{-0.0003}$ of $\vec{\gamma}^0$ projected onto the plane defined by
the  \citetalias{1999PASP..111...63F}-derived
$a_X$ and $b_X$, the quadratic fraction of $\vec{\gamma}^1$ projected
onto the dust plane is
$0.959^{+0.015}_{-0.029}$.
The   $\vec{\gamma}$  vectors project  onto $R^F_{\mathit{eff}}=2.40^{+0.15}_{-0.15}$.
This  is consistent with the $R_V=2.5$ found in the supernova samples of \citet{2011ApJ...729...55F}, and the
center  of the range of per-supernova $R^F$ values found in the samples of \citet{2014ApJ...789...32B, 2015MNRAS.453.3300A}.
Based on these facts, the supernova ``extrinsic'' colors $\vec{\gamma}^0 g_0 + \vec{\gamma}^1 g_1$  of our model are associated dust extinction.
Further discussion on this association is given in the discussion of $g_0$ and $g_1$  in \S\ref{g0g1:sec}.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{
   Visualization of how supernova magnitudes can vary in Model~III IIand that of \citetalias{1999PASP..111...63F}.  While the models describe
   magnitudes in 5-bands ${\hat{U}}{\hat{B}}{\hat{V}}{\hat{R}}{\hat{I}}$ this visualization shows only ${\hat{U}}{\hat{V}}{\hat{I}}$.   The left and right plots show the same information from
   two different perspectives.
   Our model is shown in solid-line unit vectors in the directions of $\vec{\gamma}^0$ and $\vec{\gamma}^1$. The only possible magnitudes
   are a linear combination of these two vectors, and hence are confined to the plane that contains both.
   ...
   in the directions of  $\vec{a}$, $\vec{b}$; the only possible magnitudes are confined to the plane that contains both.
   The perspective on the right visually shows that all four vectors are nearly coplanar.  
   The combination $\vec{a}+\vec{b}/2.40$ is shown in the dotted red
   line: it is almost perfectly superimposed on $\vec{\gamma}^0$.
   \label{plane3:fig}}
\end{figure}


\subsubsection{$\sigma_p \vec{\phi}$: Cofactor of Intrinsic Latent Parameter}

The posterior shows a significant color influence of the $p$ parameter.
The minimum value for $\sigma_p$ over all Monte Carlo chains is 0.039~mag and its 
68\% credible interval is $\sigma_p=0.049^{+ 0.004}_{-0.003}$~mag. 
This is significantly larger than the median data color uncertainty of 0.010~mag (the maximum is 0.042~mag).
The evaluation of 100 simulated null data sets with no third color parameter produces no median $\sigma_p$'s  as
...
a reddening $\hat{B}-\hat{V}$ is associated with a bluing in $\hat{V}-\hat{R}$.  The color changes are largest
in the extreme $\hat{U}$ and $\hat{I}$ bands. Absorption and re-emission of light
over broad bands are the most obvious candidate for producing this behavior.
This kind of behavior is already associated with SNe~Ia.
...
We emphasize that
although the $p\sigma_p \vec{\phi}$ colors have ben referred to as ``intrinsic'' due to its placement in the model, the analysis cannot quantitatively distinguish
whether its origin is intrinsic or due to a third dust parameter or some other extrinsic effect.

\subsubsection{Per-Supernova Parameter Results}
\label{results3per:sec}
Each supernova is described by its parameters $\Delta$, $EW_{Ca}$, $EW_{Si}$, $\lambda_{Si}$, $x_1$,
$E_{\gamma^0}({\hat{B}}-{\hat{V}})=(\gamma^0_{\hat{B}}-\gamma^0_{\hat{V}})g_0$, $E_{\gamma^1}({\hat{B}}-{\hat{V}})=(\gamma^1_{\hat{B}}-\gamma^1_{\hat{V}})g_1$, and $p\sigma_p\phi_{\hat{I}}$.
...
\begin{multline}
Cor(\Delta, EW_{Ca}, EW_{Si}, \lambda_{Si}, x_1, E_{\gamma^0}({\hat{B}}-{\hat{V}}), E_{\gamma^1}({\hat{B}}-{\hat{V}}), p\sigma_p\phi_{\hat{I}} ) =\\
\begin{pmatrix}
\begin{array}{rrrrrrrr}
\Delta & -0.07^{+0.06}_{-0.06} & -0.11^{+0.08}_{-0.07} & -0.17^{+0.06}_{-0.06} & 0.19^{+0.08}_{-0.08} & 0.08^{+0.07}_{-0.06} & 0.23^{+0.08}_{-0.08} & -0.13^{+0.06}_{-0.06} \\
\ldots  & EW_{Ca} & 0.09^{+0.03}_{-0.03} & -0.26^{+0.03}_{-0.03} & 0.07^{+0.03}_{-0.03} & -0.11^{+0.05}_{-0.05} & -0.18^{+0.06}_{-0.06} & -0.00^{+0.08}_{-0.07} \\
\ldots & \ldots &EW_{Si} & -0.12^{+0.03}_{-0.03} & -0.84^{+0.01}_{-0.01} & -0.13^{+0.04}_{-0.04} & 0.27^{+0.08}_{-0.09} & -0.00^{+0.08}_{-0.08} \\
\ldots  & \ldots  & \ldots  &  \lambda_{Si} & 0.14^{+0.03}_{-0.03} & 0.01^{+0.05}_{-0.05} & 0.00^{+0.06}_{-0.06} & 0.00^{+0.08}_{-0.08} \\
\ldots  &\ldots  &\ldots & \ldots & x_1 & 0.13^{+0.04}_{-0.04} & -0.20^{+0.09}_{-0.09} & 0.00^{+0.08}_{-0.08} \\
\ldots  &\ldots  & \ldots  &\ldots  & \ldots  & E_{\gamma^0}({\hat{B}}-{\hat{V}})& 0.04^{+0.06}_{-0.06} & -0.00^{+0.07}_{-0.08} \\
\ldots  & \ldots &\ldots  & \ldots  & \ldots & \ldots  &E_{\gamma^1}({\hat{B}}-{\hat{V}})  & 0.00^{+0.07}_{-0.07} \\
\ldots  & \ldots & \ldots & \ldots  & \ldots  & \ldots  &\ldots  & p\sigma_p\phi_{\hat{I}} \\
\end{array}
\end{pmatrix}.
\end{multline}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{
   Distributions for the supernova parameters $\Delta$, $EW_{Ca}$, $EW_{Si}$, $\lambda_{Si}$, $x_1$, $E_{\gamma^0}({\hat{B}}-{\hat{V}})$,  $E_{\gamma^1}({\hat{B}}-{\hat{V}})$,  and  $p\sigma_p\phi_{\hat{I}}$, as well as the grey offset
...
\subsubsection{$g_0$, $g_1$: Extrinsic Parameters}
\label{g0g1:sec}

There are $>2 \sigma$ correlations between our extrinsic color-excess parameters $g_0$, $g_1$
(equivalently $E_{\gamma^0}({\hat{B}}-{\hat{V}})$,  $E_{\gamma^1}({\hat{B}}-{\hat{V}})$) and the input features  $EW_{Ca}$,
$EW_{Si}$, and $x_1$.  This translates to correlations between dust and the input features, given the association
of  $\vec{\gamma} g$ with dust.
The correlation of $x_1$ with with host-galaxy (including dust) properties is known 
\citep{2000AJ....120.1479H, 2003MNRAS.340.1057S}.


The per-supernova parameter $\gamma^0_{\hat{V}} g_0$ can be associated with the $\hat{V}$-band
extinction for an effective $R^F_{\mathit{eff}}=2.40$ dust, as
$\vec{\gamma}_0$  has nearly the same effect on color as dust extinction at that constant $R^F$.
...
(Subtracting the value of the one supernova suppresses correlated errors in $g_0$ common to all supernovae.)
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{../output_fix3_x1/deltagamma1_med.pdf}
      ...
      Normalized stack of the posteriors of all supernovae  and histogram 
      of  per-supernova 
median values of: (left) $\gamma^0_{\hat{V}} g_0-\gamma^0_{\hat{V}} g_0|_0$, which is associated with the $A^F_V$ of an $R^F_{\mathit{eff}} \sim 2.4$ dust model;
...
An instructive way to consider the  $\{g_0, g_1\}$ parameters
is through their transformation to  $\{A_V^F, E^F(B-V)\}$ (modulo additive constants)   through the
matrix in  Eq.~\ref{trans_I:eqn}.
A plot showing the expected values and 68\% credible intervals of these parameters
for our supernova
sample is shown in Figure~\ref{kk:fig}. 
For reference, a line that represents $R^V_{\mathit{eff}}=2.4$ is overplotted.  The majority of supernovae lie within a narrow range above the line
...
The negative tail in  $\gamma^1_{\hat{V}} g_1-\gamma^1_{\hat{V}} g_1|_0$ of the right plot of Figure~\ref{k0_med:fig} is directed toward smaller $R^F$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=4in]{../output_fix3_x1/avebv_synth.pdf}
      ...
      Overplotted is a line with the slope expected for $R^F=2.4$.
   \label{kk:fig}}
\end{figure}

\subsubsection{$p$: Intrinsic Parameter}
\label{p:sec}

There is no correlation between the  intrinsic parameter $p$ and the other features.
The correlation coefficients between the ``intrinsic''  ($p\sigma_p\phi_{\hat{I}}$) and ``extrinsic''
...
The distributions of extrinsic, $E_\gamma({\hat{B}}-{\hat{V}}) \equiv (\gamma^0_{\hat{B}} -\gamma^0_{\hat{V}}) g_0
+ (\gamma^1_{\hat{B}} -\gamma^1_{\hat{V}}) g_1$, and intrinsic, $E_p({\hat{B}}-{\hat{V}}) \equiv   p \sigma_p (\phi_{\hat{B}}-\phi_{\hat{V}})$,
sources of color excess 
  are shown in Figure~\ref{ebv:fig} as the  stack of the posteriors of all supernovae 
 and the histogram of per-supernova median values.
...
%-----
mag respectively.
The latter is comparable to its measurement uncertainty per-supernova.
Previous analyses confined to $\hat{B}$ and $\hat{V}$ colors would have difficulty in detecting the intrinsic term.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4in]{../output_fix3_x1/ebv.pdf}
 %  \includegraphics[width=2.8in]{../output_fix3_x1/ebv_delta.pdf}
      \caption{Left:  Stack of the posteriors of all supernovae of the external $E({\hat{B}}-{\hat{V}})$ 
   ...
   Right:  Stack of the posteriors of all supernovae  and histogram of the per-supernova
...
For supernovae with host-properties from Childress, we plot $p$ as a function of host-galaxy mass.
$  -0.0186 \pm {    0.0034 }$
$  -0.0293 \pm {    0.0033 }$
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4in]{../output_fix3_x1/childress.pdf} 
   \caption{Intrinsic parameter
 $p$ as a function of host-galaxy mass
    \label{childress:fig}}
\end{figure}


\subsubsection{$\Delta$: Magnitude Offset}
\label{Delta:sec}
...
visual interpretation, we remove much of this
correlation by
selecting an arbitrary supernova and subtracting out its $\Delta|_0$ from those of all other objects, at the level of each MCMC link.
...
There are $>2 \sigma$ correlations between $\Delta$ and each of $\lambda_{Si}$, $x_1$, $E_{\gamma^1}({\hat{B}}-{\hat{V}})$, and
$p\sigma_p\phi_{\hat{I}}$, showing that in addition to standardizing colors, these features can also be used to standardize
absolute magnitudes.   
...
Figure~\ref{pvalues_delta2:fig} shows  that for 
 $-1$, 0, and $1$-sigma
values of  the credible interval, only
\color{blue}
2--3\%
\color{black}
of the simulations have values of  $\delta_\perp^2/\delta^2$ as low as our data.
The $p$-values $\ge 0.97$ indicate that 
...
within the analysis
\color{black}
are measured through simulation.
...
\subsection{Per-Supernova Parameter -- Intrinsic Covariance Matrix Degeneracy}
\label{degenerate:sec}
\color{blue}
As discussed in \S\ref{modelmodel:sec}, our two models have degeneracy between per-supernova parameters and the covariance matrix.
In the posterior, the degeneracy is broken by the determinant of the covariance matrix in the likelihood and by the priors.
The posterior is thus sensitive to the prior, which being most likely biased at some level, means that the
intrinsic color parameters, $\Delta$, $k$, and $D$, and intrinsic dispersion $C_c$ are susceptible to the misassignment between each other.
The total variance and the allocation of that variance between the eigenvectors of $C_c$ in particular influence the location of the posterior
maximum, and their distributions are shown in 
\S\ref{priorappendix:sec}.  Given our particular interest in the new color term, in this Subsection we consider the possibility that
$\delta$ is an artifact of leakage from intrinsic covariance.

In our model, some intrinsic dispersion could be attributed to  $\delta$.  If this were the case, one would expect for the dominant eigenvector
of $C_c$ to be aligned with $\delta$, as the corresponding eigenvalue decreases in exchange for the
artificial creation of $D$.  No such alignment is seen: the median dot products between unit $\delta$ and the ordered five $C_c$ eigenvectors
are 0.48, 0.53,
0.34,
0.24,
0.31.

The other possibility is for $\gamma^0$ and $\gamma^1$ to leak into $\delta$.  In Model~I, an estimate of leakage from
$\gamma$  into intrinsic covariance is $0.00091_{- 0.00018}^ {+ 0.00023}$~mag$^2$,
the variance in the  three  eigenvectors otherwise unconstrained in the model.    (The cumulative variances in successive
eigenvectors are shown in Figure~\ref{ccvar:fig}.)

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
%   \includegraphics[width=2.8in]{ccnvar.pdf}
   \includegraphics[width=5in]{ccvar.pdf}
      \caption{Left: Right: \label{ccvar:fig}
      }
\end{figure}


The change in the variance captured the intrinsic dispersions of Model~I and II is 0.002 mag$^2$,
as shown in Figure~\ref{ccvar:fig}.  This bounds the amount of leakage 


The nature of the degeneracy is that each intrinsic SN parameter can be exchanged with one dimension of the intrinsic covariance
matrix; given that
Model~I  has three intrinsic vectors $\Delta$, $\gamma^0$, and $\gamma^1$,
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{childress.pdf}
   \includegraphics[width=2.8in]{childress_pvalue.pdf}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{ccorr1e.pdf}
   \includegraphics[width=2.8in]{ccorr2e.pdf}
      \caption{
...
filters are used) $\beta_{\mathit{int}}=2.3\pm 0.3$ found by \citetalias{2017ApJ...842...93M}.
...
$\vec{\gamma}^0$, $\vec{\gamma}^1$, and $\sigma_p\vec{\phi}$
are given in Table~\ref{global2:tab}.
Figure~\ref{global5:fig} shows that the $\hat{I}$-band has
the largest separation between the degenerate solution of $\sigma_p \phi_X$.  To break the degeneracy we enforce a positive value for
$\phi_{\hat{I}}$, 
that is $\vec{\phi} = \text{sign}(\phi_{\hat{I}}) \vec{\phi}$.
