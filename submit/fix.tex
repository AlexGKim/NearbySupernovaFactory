...
For Model~II described in \S\ref{modelII:sec}, we add a third latent parameter to the analysis.
Light-curve shape is then added to constitute our final Model~III in \S\ref{modelII:sec}.}
...
The 
spectroscopic-feature measurements, \added{SALT2 light-curve shape parameters  $x_1$,}
...
and two extrinsic color parameters. The purpose of this section is to introduce some basic concepts of our models,
to show how some of the  model parameters can be expressed in terms of dust-extinction models, and to give Model~I results on dust extinction. 
To avoid repetitiveness,
we refrain from going into full detail on Model~I in deference to Model~III, which is presented in Section~\ref{modelIII:sec}
...
\added{With this grey offset, the model standardizes colors without reference to an absolute magnitude.
...
\added{Except where noted above, we adopt a flat prior for all parameters.
...
\subsection{Results -- $\vec{\gamma}^0$ and $\vec{\gamma}^1$:  Cofactors the of Extrinsic Color Parameters}
\label{results1:sec}
...
We  interpret our model $\gamma$'s in terms of
the dust-extinction model of \citet{1999PASP..111...63F} \citepalias[henceforth referred to as][]{1999PASP..111...63F}.

The 68\% credible intervals for $\gamma^0$ and $\gamma^1$ (and all other global parameters) are given in \S\ref{m12results:sec}.}
...
\added{Dust-extinction
models are also approximately described by a linear model, here 
using the commonly-used parameterization of $A_V$ and $E(B-V)$
\begin{equation}
...
\added{Our model $\vec{\gamma}$ vectors can be written as a linear combination (described by the $2 \times 2$ matrix M) of the
...
\label{trans_I:eqn}
\end{equation}
The $\vec{\gamma}$'s are almost entirely captured by the first term. 
A quadratic fraction of $0.9993^{+0.0002}_{-0.0002}$ of $\vec{\gamma}^0$ is projected onto $\vec{a}$--$\vec{b}$ plane, as is
$0.959^{+0.013}_{-0.024}$.of $\vec{\gamma}^1$.}
The allowed color
...
\added{The color vectors in this equation are associated with those of  Eq.~\ref{trans_I:eqn} by assigning $\kappa_1=M[1,2]/M[1,1]$, $\kappa_2=M[2,1]/M[2,2]$.}
...
\added{
The posterior yields $R^F_{\mathit{eff}} =2.18^{+0.14}_{-0.16}$.
...
Model~I.
\begin{itemize}
\item This model introduces two latent supernova parameters that can influence supernova colors with ``non-informative'' priors.  Fitting
...
another parameter for intrinsic color variability unrelated to dust.  In this section we  focus on describing the consequences of the addition of the
new parameter.
...
assumed to be drawn from a Normal distribution with unit standard deviation.
The relative effect on each band is specified by the global parameters 
$\sigma_p\vec{\phi}$, where $\vec{\phi}$ is a unit vector.
Model~II reads as
\begin{equation}
p  \sim \mathcal{N}(0,1 ),
\end{equation}
...
use ``intrinsic'' and ``extrinsic'' as descriptive labels for $\phi$ and $\gamma$ respectively, the model cannot distinguish between
...
For $N$ supernovae there are $8N$ observables.  There are $3N$ spectral parameters, each of
$\Delta$, $g_0$, $g_1$ contributes $N-1$ parameters, there are $N$ parameters for $p$,  and $5 \times 7$ global coefficients.
For $N=172$ supernovae, there are 1376 observables and 1236  parameters.


\subsection{Results -- $\sigma_p \vec{\phi}$: Cofactor of the Intrinsic Color Parameter}
\label{results2:sec}
...
(The credible intervals of all global parameters are given in  \S\ref{m12results:sec}.)
...
the $\vec{\gamma}$ vectors; a quadratic fraction of
 $0.088^{+0.118}_{-0.063}$ of 
...
Model~II introduces a new parameter whose distribution comes from a Normal distribution.
It is thus interesting to compare the distributions of the $\gamma^0_{\hat{V}} g_0$ and $\gamma^1_{\hat{V}}g_1$ parameters of Model~I and Model~II,
as shown in Figure~\ref{kcomp:fig}.  (Here and in future plots, the parameter value of one supernova,
e.g.\ $\gamma^0_{\hat{V}} g_0|_0$, is subtracted
out to null our some of the correlated error between supernovae.) The apparent difference between Model~I and II is in  $\gamma^1_{\hat{V}}g_1$ term,
where it seems that a Normal distribution component in Model~I is  assigned to $p$ in Model~II.
...
The direction of the  $\vec{\gamma}^0$ vector corresponds to an effective $R^F_{\mathit{eff}} = 2.54^{+0.20}_{-0.20}$.
...
Model~III explicitly includes light-curve shape as an additional feature used to standardize supernova colors, in contrast to
the first two models that use spectral features.
This model is of primary interest
...
the addition of the $x_1$ observation, parameter, and its implicit inclusion in the covariance matrix $C$, in Eq.~\ref{dust3:eqn}.

For $N$ supernovae there are $9N$ observables.  There are $4N$ spectral and light-curve shape parameters, each of
$\Delta$, $g_0$, $g_1$ contributes $N-1$ parameters, there are $N$ parameters for $p$,  and $5 \times 8$ global coefficients.
For $N=172$ supernovae, there are 1548 observables and 1413  parameters.
}
...
colors at peak. \citet{2009ApJ...699L.139W, 2011ApJ...729...55F} find a connection between $v_{Si}$ and color.
The non-zero values for $\vec{\zeta}$ demonstrate that light-curve shape $x_1$ is also an indicator
of broadband colors.
...
whether its origin is intrinsic or due to a third dust parameter or some other extrinsic effect.  Amalgamation
...
\label{p:sec}
By definition $p$ has a Normal distribution, which is shown, in
terms of the physically meaningful color-excess $E_p({\hat{B}}-{\hat{V}}) \equiv   p \sigma_p (\phi_{\hat{B}}-\phi_{\hat{V}})$ 
as the  stack of the posteriors of all supernovae in Figure~\ref{ebv3:fig}.  Also shown for comparison is the distribution of
...
The extrinsic color variation is much larger than that of the intrinsic and it is from the simultaneous analysis of an ensemble of objects that the intrinsic
color is discerned.
...
There is no significant correlation between the  intrinsic parameter $p$ and the other features including the extrinsic parameters.
Their independence implies that two physical parameters are not being artificially attributed to three
...
the combined effect of the color terms $g_0$, $g_1$, and $p$.  
...
(The absence of an intrinsic color-magnitude relation corresponds to $\beta_{\mathit{int}}=0$.)
...
In our Model~III, each supernova has three color parameters that can distinguish between
...
${\hat{U}}$-band magnitude corrections from the three color parameters $\gamma^0_{{\hat{U}}} g_0 +\gamma^1_{{\hat{U}}} g_1 + \sigma_p \phi_{\hat{U}} p$
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4in]{../output_fix3_x1/ccorr1e.pdf}
  % \includegraphics[width=2.8in]{../output_fix3_x1/ccorr2e.pdf}
      \caption{
      The ${\hat{U}}$-band magnitude corrections from the three color parameters $\gamma^0_{{\hat{U}}} g_0 +\gamma^1_{{\hat{U}}} g_1 + \sigma_p \phi_{\hat{U}} p$
...
This statistic is of interest, as a correlation between Hubble residual and host-galaxy mass
...
Indeed  \citetalias{2017ApJ...842...93M} find that when introducing intrinsic color scatter as a latent parameter, the null mass-step effect becomes more probable.
The intrinsic $p$ parameter is a possible supernova tracer of a population that to date has only been identified through host-galaxy tracers.
...
%---
$\langle \sigma_p\phi_{\hat{I}}(p-p|_0) \rangle=   -0.0186 \pm {    0.0034 }$ mag,
$\langle \sigma_p\phi_{\hat{I}}(p-p|_0)  \rangle= -0.0293 \pm {    0.0033 }$ mag
for low- and high-mass hosts respectively, making a difference is $ 0.0141 \pm {    0.0037 }$~mag, significant at almost $4 \sigma$.
Low- and high-mass galaxies ikely
host supernovae with different $p$ distributions.  
The two-tailed $p$-value of the Kolmogorov-Smirnov test is $ 0.02 _ {   -0.02 } ^{   + 0.04 }$.


\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=4in]{../output_fix3_x1/childress.pdf} 
   \caption{Intrinsic parameter $\sigma_p\phi_{\hat{I}}p$  versus host galaxy mass. Overplotted are the mean and 1$\sigma$ uncertainty on the mean for supernovae with hosts
      less than and greater than  $\log{(M/M_\sun)}=10$.
 $p$ as a function of host-galaxy mass
    \label{childress3:fig}}
\end{figure}

This article is confined to the standardization of SN~Ia colors.  Magnitude corrections related to our intrinsic $p$
parameter that may eliminate the mass step are not pursued.  We confine ourselves to noting the differences in the $p$ distributions of
low- and high-mass galaxies, and highlighting that there is room for magnitude corrections
as seen in the correlation between $\Delta$ and $p\sigma_p\phi_{\hat{I}} $ in Eq.~\ref{corr:eqn}.

\subsubsection{$\Delta$: Magnitude Offset}
\label{Delta:sec}

The color-standardization leaves magnitude residuals $\Delta$.
The fit $\Delta$'s are  correlated between supernovae so we consider
their values elative to that of an arbitrary supernova  $\Delta_0$.
...
distribution of $A_V$.  
The data show the need for at least two color parameters that can be attributed to a broad distribution  of
$R^F$ parameter values,
with an effective $R^F_\mathit{eff}=2.4$ and with an infrequent but extended scatter toward lower values.
A significant bias in $R^F$ is obtained when the possibility of another source of color variation
is unaccounted for.
We identify a new
...
of those samples that are  consistent with our $\sigma_p \vec{\phi}$ would provide strong evidence for  an
observational signature of the intrinsic SN color-magnitude relationship.

The current data shows a statistically significant difference in the values of our new parameter for supernovae
...
NIR light-curve properties and their relation with colors, host properties, and magnitudes.  Measuring the
correlation between our parameter and infrared light curves  is also planned for future work.
