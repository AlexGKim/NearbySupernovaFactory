...
\added{three color parameters and dependence on the equivalent widths of Ca~II and Si~II, the Si~II velocity,
...
Correspondingly the sample has an effective value of $R^F_{\mathit{eff}} \sim 2.40$, though with a broad distribution of free
...
\added{With this grey offset the model standardizes colors, not absolute magnitude.
...
\added{All parameters not  otherwise noted above have flat priors.
...
\subsection{Results for the Extrinsic Latent Parameter Cofactors  $\vec{\gamma}^0$ and $\vec{\gamma}^1$:  }
\label{results1:sec}
...
for our two extrinsic color terms to capture the effects of dust.
...
our Monte Carlo chains for $\vec{\gamma}$ come close to the origin.   We claim detection
with probability $1-5\times 10^{-5}$ a magnitude term in the form of a 5-dimensional vector (for the 5 bands) spanned by two basis vectors
\begin{equation}
...
Table~\ref{global3:tab} shows there the largest signal-to-noise in
in $\sigma_p \phi_{\hat{V}}$.
...
\begin{multline}
Cor(\Delta, EW_{Ca}, EW_{Si}, \lambda_{Si}, x_1, E_{\gamma^0}({\hat{B}}-{\hat{V}}), E_{\gamma^1}({\hat{B}}-{\hat{V}}), \sigma_p\phi_{\hat{V}}p ) =\\
\begin{pmatrix}
...
\label{p:sec}
The model assumes that $p$ has a Normal distribution, which is shown in
terms of the color-excess $E_p({\hat{B}}-{\hat{V}}) \equiv   \sigma_p (\phi_{\hat{B}}-\phi_{\hat{V}})p$ 
...
For data, they use the outputs of SALT2 fits: the $c$ color parameter associated with but not exactly $B-V$; the
supernova absolute magnitude $M$ associated with but not exactly $B$.  In the \citetalias{2017ApJ...842...93M}
model, the color contributes a linear   $\beta_{\mathit{int}} c$ the absolute
magnitude.
They find a significant $\beta_{\mathit{int}} = 2.2\pm 0.3$.
While we also claim a detection of an intrinsic color parameter, the differences between our models and results make it
...
is $\phi_{\hat{B}}/(\phi_{\hat{B}}-\phi_{\hat{V}}) = -1.9^{+1.0}_{-2.4}$, which appears to be inconsistent with  \citetalias{2017ApJ...842...93M}.  
However a change of the color baseline gives a significantly different
$\phi_{\hat{B}}/(\phi_{\hat{B}}-\phi_{\hat{R}}) =2.2^{+2.1}_{-0.9}$.  Our results indicate that $\beta_{\mathit{int}}$
is sensitive to the color that $c$ corresponds to.  Although SALT2 $c$ is associated with $B-V$, it is
determined using data from all bands with the wavelength extremes  typically providing the strongest leverage.
The SALT2 $c$ parameter is an  amalgamation of all input colors.
We therefore make no conclusions on the consistency between our intrinsic-color properties
and those of \citetalias{2017ApJ...842...93M}.
...
reduces the strength of  the mass-step.
...
Low- and high-mass galaxies likely
host supernovae with different $p$ distributions, as
...
%---
$\langle \sigma_p\phi_{\hat{V}}(p-p|_0) \rangle=  0.023 \pm {     0.004}$ mag,
$\langle \sigma_p\phi_{\hat{V}}(p-p|_0)  \rangle=0.036 \pm {     0.004 }$ mag
for low- and high-mass hosts respectively, making a difference is $0.013 \pm {     0.005 }$~mag, significant at $>2 \sigma$.
While we make an association between $p$ and host mass, 
we refrain from directly associating $p$ with absolute magnitude corrections.
We preview the discussion in \S\ref{Delta:sec} by mentioning that there is room for $p$-based magnitude corrections
as seen in the correlation between $\Delta$ and $\sigma_p\phi_{\hat{V}}p$ in Eq.~\ref{corr:eqn}.
...
their values relative to that of an arbitrary supernova  $\Delta_0$.
...
absolute magnitudes.   
The focus of this article is color standardization so absolute magnitude standardization his is beyond the scope of this article
...
distribution of $A_V$.  
We find a broad distribution for $R^F$ with an effective $R^F_\mathit{eff}=2.4$ and an asymmetric tail extending toward lower values.
We identify a new
...
that we and  \citetalias{2017ApJ...842...93M}  are seeing the effect of the same intrinsic color and for an
...
NIR light-curve properties and their relation with colors, host properties, and  magnitudes.
