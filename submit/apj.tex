...
with units of mag) and slopes (with units mag~\AA$^{-1}$)
of the linear relationships that
relate spectral-feature parameters with intrinsic magnitudes.
The global parameter vectors $\vec{\gamma}^0$, $\vec{\gamma}^1$  are the slopes (with units of mag) that relate 
...
Leakage from per-supernova intrinsic color parameters to intrinsic dispersion could bias the intrinsic color vectors of the SN Ia family,  or conversely, intrinsic dispersion could be spuriously attributed to an intrinsic color.  We examine this effect in the discussion of the results in \S\ref{degenerate:sec}.
...
correlation matrix with a $\eta=4$  LKJ prior
...
\color{blue}
Further properties of the this prior are presented in \S\ref{priorappendix:sec}.
\color{black}
...
\subsection{Per-Supernova Parameter -- Covariance Matrix Degeneracy}
...
Given that the priors we use are not exactly correct,
the parameters are susceptible to the misassignment, in either direction, between intrinsic color, $\Delta$, $k$, and $D$, and intrinsic dispersion $C_c$.
\S\ref{priorappendix:sec}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \includegraphics[width=2.8in]{ccvar.pdf}
      \caption{Left: Right: 
      }
\end{figure}
...
\appendix
\section{Intrinsic Covariance Matrix Prior}
\label{priorappendix:sec}
\color{blue}
The covariance matrix $C_c$
is parameterized in two parts.
The variance for each of the 5 bands comes from a  truncated $\text{Cauchy}(0.1,0.1)$ distribution
restricted to positive values.
The correlation matrix of $C_c$ is
drawn from an LKJ prior \citep{Lewandowski20091989}
that has the property
\begin{equation}
p(\text{corr}(C_c)| \eta) \propto \text{det}(\text{corr}(C_c))^{\eta-1}.
\end{equation}
For $\eta <1$, smaller determinants (stronger correlations)  are likely, for $\eta>1$ larger determinants (weaker correlations)
are likely, 
and $\eta=1$ the density is uniform.  For $\eta=\infty$ the prior is of a diagonal  matrix with no correlation.
We choose $\eta=4$ for our prior.

The distribution of determinants from which the covariance matrices are drawn is visualized in the left plot of Figure~\ref{priordet:fig},
which shows
histograms of $\ln{(\text{det}(C_c))}$  realizations for different $\eta$ priors.
The distribution has a steep drop that counteracts the log-likelihood's linear increase with decreasing
log-determinant.  There is little qualitative difference in the distributions from $\eta$'s ranging from 0.5 to 20.
The diagonal prior  produces significantly higher determinants.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=2.8in]{priordet.pdf}
   \includegraphics[width=2.8in]{prior.pdf}
         \caption{Left: Histograms of $\ln{(\text{det}(C_c))}$ from realizations for different $\eta$ priors.
         Right: Histograms of the normalized variance captured by the leading $n$ eigenvectors for $\eta=0.5$, 20,
         and a diagonal prior.
   \label{priordet:fig}}
\end{figure}

The distribution of the variance among the matrix eigenvectors is visualized in  right plot of Figure~\ref{priordet:fig},
which shows histograms
of the normalized variance captured by the leading $n$ eigenvectors for different $\eta$ priors.
The variance is  broadly distributed in the diagonal case, whereas for finite $\eta$ variance is concentrated
in fewer eigenvectors. As with the
determinants, there is little qualitative difference in the distributions from $\eta$'s ranging from 0.5 to 20.
