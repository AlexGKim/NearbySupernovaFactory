...
pp = PdfPages("temp/plane0BVR.pdf")
plt.tight_layout()
...
ax.yaxis.set_ticks(numpy.arange(-.75,.76,.25))
ax.xaxis.set_ticks(numpy.arange(-.5,.76,.5))
pp = PdfPages("temp/plane1BVR.pdf")
plt.tight_layout()
plt.savefig(pp,format='pdf')
pp.close()
plt.close()
...
plt.legend(loc=4)
pp = PdfPages("temp/avebv_synth.pdf")
plt.savefig(pp,format='pdf')
pp.close()
plt.close()
...
# plt.ylim((-1,5))
# pp = PdfPages("temp/avrv_synth.pdf")
# plt.savefig(pp,format='pdf')
# pp.close()
# plt.close()
