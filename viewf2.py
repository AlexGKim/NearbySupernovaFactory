...
mega = numpy.array([fit['c'],fit['alpha'],fit['beta'],fit['eta'],fit['gamma'],fit['gamma1'],fit['rho1']])
mega = numpy.transpose(mega)
...
for index in xrange(5):
    figure = corner.corner(mega[index,:,:],labels=[r"$c_{}$".format(cname[index]), r"$\alpha_{}$".format(cname[index]),\
                    ...
                    r"$\gamma^1_{}$".format(cname[index]),r"$\delta_{{{}}}$".format(cname[index])], \
                    truths=[None,0,0,0,0,0,0],label_kwargs={'fontsize':22})
...
fig = plt.figure()
ax = fig.add_subplot(111)
ax.errorbar(1e4/efflam,y,yerr=[y-ymin,ymax-y],fmt='o')
ax.axhline(0,linestyle=':')
...
ax.set_ylim((-1.5,1.2))
plt.tight_layout()
pp = PdfPages(dirname+'/deltaratiov2.pdf')
plt.savefig(pp,format='pdf')
pp.close()
plt.close()
